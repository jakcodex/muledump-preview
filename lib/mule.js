...
// dom snippet generators
    ...
    function addmainbutton(mule, target) {
        ...
        if ( window.mulelogin ) {
            ...
            l.text('\u21d7');
            l.attr('href', mulelink(mule.guid));
...
// Mule object
    ...
    Mule.prototype.query = function(ignore_cache, cache_only, freshness, MuleQueue) {
        ...
        function onFail() {
            ...
            if (self.fails < setuptools.config.muledump.corsMaxAttempts) {
                ...
                self.queueFinish(self.guid, 'error', false, 'CORS');
        ...
        function onResponse(xml, textStatus, request) {
            ...
            if ( !request.getResponseHeader('X-Jakcodex-CORS') ) {
                setuptools.app.muledump.notices.add('New CORS Extension Available', setuptools.app.assistants.jakcodexcors);
            } else setuptools.state.extension = true;
            ...
            if ( error = xml.match(/^<Error\/?>(.*?)<\/?Error>$/) ) {
                ...
                if ( error[1] === 'Internal error, please wait 5 minutes to try again!' ) {
                    ...
                    self.queueFinish(self.guid, 'rateLimited');
    ...
    Mule.prototype.parse = function(data, skipCacheWrite) {
        ...
        function calculateStatTotals(data, guid) {
            ...
            if ( typeof data.SalesForce.last_purchase_date === 'string' ) {
                result.LastGoldPurchase = data.SalesForce.last_purchase_date;
                result.LastGoldPurchase = result.LastGoldPurchase.splice(2, 0, '-');
                result.LastGoldPurchase = result.LastGoldPurchase.splice(5, 0, '-');
            }
        ...
        if ( skipCacheWrite !== true ) {
            try {
                window.techlog('Muledump/DataCache writing to ' + this.cache_id(), 'force');
        ...
        if (this.opt('accountinfo')) {
            ...
            $('<div>\
                ...
                <br>Last Gold Purchase Date: ' + stats.LastGoldPurchase + ' \
