...
$(function() {
	...
	$ex.on('click.muledump.exportMenu', 'div', export_totals);
});
	...
	render_totals(function(r) {
        ...
        req.always(function() {
            ...
            if (!data.success || !data.data) {
                ...
                setuptools.app.ga('send', 'event', {
                    eventCategory: 'detect',
                    eventAction: 'export-imgur-error'
                });
                return
            }

            $.featherlight('<p>Your image link: <a href="' + data.data.link + '" target="_blank">' + data.data.link + '</a><br><br><a href="' + data.data.link + '" target="_blank"><img src="' + data.data.link + '"></a></p>');
            setuptools.app.ga('send', 'event', {
                eventCategory: 'detect',
                eventAction: 'export-imgur'
            });
        	...
        	if ( errDesc == 'timeout' ) {
                ...
                setuptools.app.ga('send', 'event', {
                    eventCategory: 'detect',
                    eventAction: 'export-imgur-networkError'
                });

			} else {

        		$.featherlight('<p>imgur upload failed: ' + xhr.status + (errDesc ? ' ' + errDesc : '') + '</p>');
                setuptools.app.ga('send', 'event', {
                    eventCategory: 'detect',
                    eventAction: 'export-imgur-httpError'
                });
