...
// dom snippet generators
    ...
    function item(id, type, num) {
        id = +id;
        if ( setuptools.tmp.constantsRemovedItems.indexOf(id) > -1 ) id = -1;
    ...
    function item_listing(arr, classname) {
        ...
        for (var i = 0; i < arr.length; i++) {
            var itemid = arr[i];
            if ( setuptools.tmp.constantsRemovedItems.indexOf(Number(arr[i])) > -1 ) itemid = -1;
            item(itemid).appendTo($r);
        }
...
// Mule object
    ...
    var Mule = function(guid) {
        ...
        this.totals = new Muledump_TotalsCounter(this.guid, true, function(self) {
            ...
            if (
                setuptools.app.muledump.totals.config.getKey('accountFilter', false, []).length === 0 ||
        ...
        this.dom.on('click.muledump.mule', function(e) {
            ...
            if ( setuptools.app.muledump.totals.config.getKey('accountFilter').indexOf(self.guid) === -1 ) {
                ...
                setuptools.lightbox.tooltip(false, '<span style="padding: 0 5px;">Added!</span>', {position: {top: e.pageY-5, left: e.pageX}, heightFrom: 'tooltip', classes: 'flex-container noFlexAutoWidth'}).fadeOut(2500, function() {
                    this.remove();
                });
            } else {
                ...
                setuptools.lightbox.tooltip(false, '<span style="padding: 0 5px;">Removed!</span>', {position: {top: e.pageY-5, left: e.pageX}, heightFrom: 'tooltip', classes: 'flex-container noFlexAutoWidth', css: {padding: '0 5px;'}}).fadeOut(2500, function() {
                    this.remove();
                });
            }
    ...
    //Mule.prototype.totals = function() {
    //    return this.totals;
    //};
    ...
    Mule.prototype.parse = function(data, skipCacheWrite) {
        ...
        if ( skipCacheWrite !== true ) this.totals = new Muledump_TotalsCounter(this.guid, false, function(self) {
            if (
        ...
        for (var i = 0; i < carr.length; i++) {
            ...
            if ( this.totals.cache === false ) {
                ...
                for ( var te = 0; te < 4; te++ ) this.totals.count(eq[te] || "-1", 'equipment');
                for ( var ti = 4; ti < 12; ti++ ) this.totals.count(eq[ti] || "-1", 'inv');
                if ( +c.HasBackpack === 1 ) for ( var tb = 12; tb < ( (eq.length < 20) ? 20 : eq.length ); tb++ ) this.totals.count(eq[tb] || "-1", 'backpack');
        ...
        if ( this.totals.cache === false ) this.totals.save();
