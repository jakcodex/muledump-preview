...
// dom snippet generators
    ...
    function item_listing(arr, classname, classid) {
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            item(itemid, undefined, undefined, [classname, i, classid]).appendTo($r);
        }
    ...
    function addmainbutton(mule, target) {
        ...
        if ( mule.guid.match(setuptools.config.regex.guid) === null ) {
            ...
            l.attr('href', '#');
            if ( !window.mulelogin ) l.css({display: 'none'});
            l.appendTo(target);

            var stage = 0;
            l.on('click.mulelogin', function(e) {

                $(this).attr('href', setuptools.app.muledump.mulelink(mule.guid));
...
// Mule object
    ...
    Mule.prototype.parse = function(data, skipCacheWrite) {
        ...
        if (this.opt('classstats') && d.Account && d.Account.Stats && d.Account.Stats.ClassStats && d.Account.Stats.ClassStats.length > 0 ) {
            ...
            var cswidth = (setuptools.data.config.rowlength+1)*210;
            var cshtml = ' \
        ...
        for (var i = 0; i < carr.length; i++) {
            ...
            if (this.opt('equipment') || this.opt('inv') || dobp) {
                ...
                if (this.opt('equipment')) {
                    ...
                    if ( this.loginOnly === false && !this.opt('shrink') ) itc.append(item_listing(eq.slice(0, 4), 'equipment', c.ObjectType));
                }
                if (this.opt('inv')) {
                    tcount = tcount.concat(eq.slice(4, 12));
                    if ( this.loginOnly === false && !this.opt('shrink') ) itc.append(item_listing(eq.slice(4, 12), 'inv'));
                }
                if (dobp) {
                    tcount = tcount.concat(eq.slice(12, eq.length));
                    if ( this.loginOnly === false && !this.opt('shrink') ) itc.append(item_listing(eq.slice(12, eq.length), 'backpack'));
                }
        ...
        vaultlayout = ( this.opt('vaultlayout') ) ? this.opt('vaultlayout') : setuptools.data.options.vaultlayout;
        vaultwidth = (
