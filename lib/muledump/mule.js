...
// dom snippet generators
    ...
    function addmulemenu(mule) {
        ...
        var muleId = 'mule-' + setuptools.seasalt.hash.sha256(mule.guid);
...
// Mule object
    ...
    Mule.prototype.error = function(s) {
        ...
        //  add error to error bar
        this.errorBar.html(s);
    ...
    Mule.prototype.query = function(ignore_cache, cache_only, freshness, MuleQueue) {
        ...
        setuptools.app.uaTimer('mule', 'realmAPICharListGet', 'start', this.guid);
        window.realmAPI('char/list', CR, function(xhr) {
            xhr.done(onResponse).fail(onFail)
        });

        function onFail() {
            setuptools.app.uaTimer('mule', 'realmAPICharListGet', 'cancel', self.guid);
            self.log('failed');
            self.busy = false;
            self.fails++;
        ...
        function onResponse(xml, textStatus, request) {
            ...
            setuptools.app.uaTimer('mule', 'realmAPICharListGet', 'stop', self.guid);
            ...
            if ( error = xml.match(/^<Error\/?>(.*?)<\/?Error>$/) ) {
                ...
                setuptools.app.uaTimer('mule', 'xmlToJson', 'start', self.guid);
                var JSONData = xmlToJson(parser.parseFromString(xml, "text/xml"));
                setuptools.app.uaTimer('mule', 'xmlToJson', 'stop', self.guid);
    ...
    Mule.prototype.parse = function(data, skipCacheWrite) {
        ...
        function calculateStatTotals(data, guid) {
            ...
            setuptools.app.uaTimer('mule', 'calculateStatsTotals', 'start', self.guid);
            var ign = '';
            var char = [];
            ...
            setuptools.app.uaTimer('mule', 'calculateStatsTotals', 'stop', self.guid);
            return result;
        ...
        //  start parse timer
        setuptools.app.uaTimer('mule', 'parse', 'start', this.guid);
        ...
        if ( skipCacheWrite !== true ) {
            try {
                ...
                setuptools.app.uaTimer('mule', 'writeAccountDataCache', 'start', this.guid);
                localStorage[this.cache_id()] = JSON.stringify(data, null, 4);
                setuptools.app.uaTimer('mule', 'writeAccountDataCache', 'stop', this.guid);
            } catch (e) {
        ...
        if ( this.loginOnly === false ) {
            ...
            if (this.opt('accountinfo')) {
                ...
                $('<div class="ml5">\
                Account Fame: ' + this.data.query.results.Chars.Account.Stats.Fame + ' \
                ...
                <br>Total Unlocked Chests: ' + stats.TotalChests + ' of ' + (vaultorders[0].vaultorder.filter(function (item) {
                    return (item !== 0)
                })).length + ' \
                <br>Account Data Cache Age: ' + stats.CacheAge + ' ' + ((stats.CacheAge === 1) ? 'day' : 'days') + ' \
            ...
            if (TotalGifts >= setuptools.config.giftsWarningThreshold) {
                ...
                if (
                    ...
                    <div class="flex-container">\
                        <div>Warning: Account is nearing the <a href="' + setuptools.config.giftChestsBugHelp + '" class="rateLimited nopulse" target="_blank">gift chests bug</a>!</div> \
                        <div class="setuptools link giftChestsBugAck menuStyle notice mr0 ml5" title="Acknowledge" style="width: auto; height: auto;">Close</div>\
                    </div>\
                ');

                    //  bind the acknowledgement button
                    $('div.setuptools.link.giftChestsBugAck').click(function () {

                        //  save the ack if the user has setuptools enabled
            ...
            setuptools.app.uaTimer('mule', 'parseCharacterSorting', 'start', this.guid);
            ...
            setuptools.app.uaTimer('mule', 'parseCharacterSorting', 'stop', this.guid);
        ...
        for (var i = 0; i < carr.length; i++) {
            ...
            var mainCharLoopTimer = new JTimer;
            ...
            */
            ...
            if (this.loginOnly === false && this.opt('wawawa')) {
                ...
                $table.appendTo($c);
                wawawaTimer.stop(function(timer) {
                    setuptools.tmp.wawawaTimers[self.guid] = setuptools.tmp.wawawaTimers[self.guid]+timer.runtime;
                });
            ...
            setuptools.app.ga('timing', {
                category: 'mule',
                key: 'parseWawawa',
                value: setuptools.tmp.wawawaTimers[self.guid]
            });

            if ( setuptools.data.config.debugging === false ) timing['mule-parseWawawa-' + self.guid] = {
                runtime: setuptools.tmp.wawawaTimers[self.guid]
            };
            // WAWAWA PART END
            /////////////////////////
            ...
            mainCharLoopTimer.stop(function(timer) {
                setuptools.tmp.mainCharLoopTimers[self.guid] = setuptools.tmp.mainCharLoopTimers[self.guid]+timer.runtime;
            });
        ...
        setuptools.app.ga('timing', {
            ...
            value: setuptools.tmp.mainCharLoopTimers[self.guid]
        });

        if ( setuptools.data.config.debugging === false ) timing['mule-parseMainCharacterLoop-' + self.guid] = {
            runtime: setuptools.tmp.mainCharLoopTimers[self.guid]
        };
        ...
        if (this.opt('vaults')) {
            ...
            setuptools.app.uaTimer('mule', 'parseVaults', 'start', this.guid);
            ...
            setuptools.app.uaTimer('mule', 'parseVaults', 'stop', this.guid);
        ...
        if (this.opt('gifts')) {
            ...
            setuptools.app.uaTimer('mule', 'parseGifts', 'start', this.guid);
            ...
            if ( typeof gifts === 'string' ) {
                ...
                if ( this.loginOnly === false ) maketable('gifts', garr, 5).appendTo(this.dom);
            }
            setuptools.app.uaTimer('mule', 'parseGifts', 'stop', this.guid);

        }

        setuptools.app.uaTimer('mule', 'parse', 'stop', this.guid);
