...
// Mule object
    ...
    var Mule = function(guid) {
        ...
        this.totals = new Muledump_TotalsCounter(this.guid, false, function(self) {
            ...
            if (
                ...
                (typeof self === 'object' && typeof self.guid === 'string' && setuptools.data.muledump.totalsAccountFilter.indexOf(self.guid) > -1)
    ...
    Mule.prototype.parse = function(data, skipCacheWrite) {
        ...
        if ( skipCacheWrite !== true ) this.totals = new Muledump_TotalsCounter(this.guid, false, function(self) {
            if (
                setuptools.data.muledump.totalsAccountFilter.length === 0 ||
                setuptools.data.muledump.totalsAccountFilter.indexOf(self.guid) > -1
            ) setuptools.tmp.globalTotalsCounter.import(self);
        });
        ...
        for (var i = 0; i < carr.length; i++) {
            ...
            if ( this.totals.cache === false ) {
                ...
                if ( dobp ) for ( var tb = 12; tb < ( (eq.length < 20) ? 20 : eq.length ); tb++ ) this.totals.count(eq[tb] || "-1", 'backpack');
