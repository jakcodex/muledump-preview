...
//old order//var SLOT_ORDER = [1,2,3,17,8,24, 14,6,7, 4,5,11,12,13,15,16,18,19,20,21,22,23,25, 9, 10, 26, 0];
//  new default order (melee, dagger, bow, staff, wand, and repeat order for abilities, armor, etc, misc at end)
var SLOT_ORDER = [1, 2, 3, 17, 8, 24, 5, 16, 12, 13, 18, 22, 15, 20, 11, 19, 21, 4, 23, 25, 7, 6, 14, 9, 10, 26, 0];
...
//  100004  //  4 - tomes
//  100005  //  5 - shields
//  100006  //  6 - light armor
//  100007  //  7 - heavy armor
//  100008  //  8 - wands
//  100009  //  9 - rings
//  110000  //  10 - potions (tier 2, bagtype 5)
//  110001  //  10 - potions soulbound (tier -1, bagtype 5)
//  110002  //  10 - skins (tier -1, bagtype 8, fp > 200)
//  110003  //  10 - dyes/cloths - (tier -1, bagtype 8, fb 0, soulbound)
//  110004  //  10 - inc - (tier -1, bagtype 8, not soulbound)
//  110005  //  10 - dyes/cloths (tier -1, bagtype 8, fp 0)
//  110006  //  10 - greater/minor hpmp (tier -1, bagtype 0)
//  110007  //  10 - hpmp (tier 1, bagtype 0)
//  110008  //  10 - alcohol (tier 3, bagtype 2)
//  110009  //  10 - elixers/tinctures/pirate rum/shrooms/coral juice (tier -1, bagtype 2, soulbound)
//  110010  //  10 - drake eggs (tier -1, bagtype 2, not soulbound)
//  110011  //  10 - effusions (tier -1, bagtype 5)
//  ^^^^^really rotmg???^^^^^
//  100011  //  11 - spells
//  100012  //  12 - seals
//  100013  //  13 - cloaks
...
function update_totals() {
	...
	function isbad(id) {
        ...
        if ( setuptools.data.muledump.totalsItemFilter.indexOf(id) > -1 ) return true;
        ...
        for (var i in mules)
            ...
            //  check if mule is disabled or if the account filter is active
                if (!(mules[i].disabled || (
					...
					setuptools.data.muledump.totalsAccountFilter.length > 0 &&
					setuptools.data.muledump.totalsAccountFilter.indexOf(i) === -1)
                ))
...
function update_filter() {
	...
	$('.mule').each(function() {
		...
		//  traditional totals filtering on displayed items in mules
        if ( setuptools.data.options.totalsGlobal === false ) {
        ...
        //  new global totals filtering
            ...
            for ( var s = 0; s < selected.length; s++ ) {
                ...
                var totals = window.mules[guid].totals.data.totals;
                if ( typeof totals[itemid] === 'number' ) {
                    $(this).css('display', 'inline-block');
                    return;
                }
