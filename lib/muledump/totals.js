...
function update_totals() {
	...
	function isbad(id) {
        ...
        if ( setuptools.data.muledump.totals.itemFilter.indexOf(id) > -1 ) return true;
        ...
        for (var i in mules)
            ...
            //  check if mule is disabled or if the account filter is active
                if (!(mules[i].disabled || (
					...
					setuptools.data.muledump.totals.accountFilter.length > 0 &&
					setuptools.data.muledump.totals.accountFilter.indexOf(i) === -1)
                ))
