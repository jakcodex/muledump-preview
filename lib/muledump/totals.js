...
function ids_sort(a, b) {
	...
	return primaryFilter(a, b) || secondaryFilter(a, idA, b, idB);
...
function update_totals() {
	...
	function isbad(id) {
	    ...
	    if (!options.famefilter && !options.feedfilter && !options.utfilter && !options.sbfilter && !options.stfilter && !options.nonsbfilter ) return false;
	...
	*/
	...
	if ( setuptools.data.options.totalsGlobal === false ) {
		...
		setuptools.app.techlog('Muledump/Update_Totals running in mode 0');
        for (var i in mules)
	...
	//  scan over the global totals items
		...
		if ( setuptools.app.muledump.totals.config.getKey('accountFilter').length === 0 ) {
            ...
            setuptools.app.techlog('Muledump/Update_Totals running in mode 1');
            ...
            setuptools.app.techlog('Muledump/Update_Totals running in mode 2');
            for (var i in mules)
                if (mules.hasOwnProperty(i))
                    if (setuptools.app.muledump.totals.config.getKey('accountFilter').indexOf(i) > -1)
                        for (var itemid in mules[i].totals.data.totals)
                            if (mules[i].totals.data.totals.hasOwnProperty(itemid))
                                upd2(itemid, mules[i].totals.data.totals[itemid]);
