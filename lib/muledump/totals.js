...
function update_totals() {
	...
	// count items
	setuptools.app.uaTiming('totals', 'update', 'start', 'aggregate');
	for (var i in mules) {
		if ( mules.hasOwnProperty(i) ) {

            var m = mules[i].items;
            if (mules[i].disabled || !m || (
            	Array.isArray(setuptools.tmp.totalsAccountFilter) === true &&
                    setuptools.tmp.totalsAccountFilter.length > 0 &&
					setuptools.tmp.totalsAccountFilter.indexOf(i) === -1)
			) continue;

            //  process char items
            for (var j = 0; j < m.chars.length; j++) upd(m.chars[j]);

            //  process vaults
            if (mules[i].opt('vaults') === true)
                for (j = 0; j < m.vaults.length; j++)
                    upd(m.vaults[j]);

			//  process gifts (yes it's just m.gifts[0] but oh well)
			if (mules[i].opt('gifts') === true)
				for (j = 0; j < m.gifts.length; j++)
					upd(m.gifts[j]);
    ...
    setuptools.app.uaTiming('totals', 'update', 'stop', 'aggregate');
