...
//  check for updates and display a lightbox with the results
    function checkupdates(force) {
        ...
        function DisplayMessage() {
            ...
            setuptools.lightbox.settitle('muledump-about', '<strong>Muledump Local v' + VERSION + ( (PATCHVERSION !== false && PATCHVERSION !== 0) ? '.' + PATCHVERSION : '' ) + '</strong>');
        ...
        function ProcessResponse(data) {
            ...
            for (var i = 0; i < d.length; i++) {
                ...
                if ( typeof rendersData === 'object' && rendersData !== null ) {
                    ...
                    if ( newTimestamp > latestRenders ) latestRenders = newTimestamp;

                }

                if ( latestRenders > currentRenders ) $('#rendersupdate').css({
                    "color": '#ffffff',
                ...
                .show()
                .unbind('click')
                .click($.extend(true, [], d[i], rendersData, {currentRenders: currentRenders, latestRenders: latestRenders}), function(e) {
                    setuptools.app.assistants.rendersupdate(e.data);
                });
    ...
    $(function() {
        ...
        $('#about').bind('click', function() {
            if ( setuptools.state.hosted === false ) {
                ...
                setuptools.lightbox.settitle('muledump-checkupdates', '<strong>Muledump Online v' + VERSION + ( (PATCHVERSION !== false && PATCHVERSION !== 0) ? '.' + PATCHVERSION : '' ) + '</strong>');
