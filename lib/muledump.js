...
$(function() {
	...
	if ( setuptools.config.devForcePoint !== 'online-versioncheck' && setuptools.state.hosted === false ) {
		...
		function alertNewVersion() {
            ...
            setuptools.lightbox.display('muledump-alertNewVersion', {variant: 'setuptools-small', closeSpeed: 0, openSpeed: 0});
            $('.drawhelp').click(function(e) {
		...
		if ( Object.keys(setuptools.data.version).length === 0 ) {
        ...
        var matches = VERSION.match(/^0\.([0-9]*?)\.([0-9]*?)$/);
		if ( setuptools.data.config.alertNewVersion === 2 ) {

			if ( matches[1] > setuptools.data.version.major || matches[2] > setuptools.data.version.minor || PATCHVERSION > setuptools.data.version.patch ) alertNewVersion();

		//  notify on new releases only
		} else if ( setuptools.data.config.alertNewVersion === 1 ) {

			if ( matches[1] > setuptools.data.version.major || matches[2] > setuptools.data.version.minor ) alertNewVersion();
