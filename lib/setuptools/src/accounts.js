...
setuptools.app.accounts.manager = function() {
    ...
    function AccountsListUpdate(direction) {
        ...
        $('div.AccountsList.list').html(' \
            ...
            ' + ( (AccountsPaginate.lastPage > 0) ? '<div class="controller enableAll formStyle" style="margin-top: 5px; clear: left; margin-right: 5px;">Enable All</div>' : '' ) + ' \
            ' + ( (AccountsPaginate.lastPage > 0) ? '<div class="controller disableAll formStyle" style="margin-top: 5px; clear: right;">Disable All</div>' : '' ) + ' \
        ');
    ...
    function AccountsContext() {
        ...
        //  enable all
        $('div.AccountsList.list > div.controller').click(function() {

            var AccountsList = setuptools.lightbox.menu.paginate.state.AccountsList.PageList;
            if ( $(this).hasClass('enableAll') === true ) {

                for ( var i in AccountsList )
                    if ( AccountsList.hasOwnProperty(i) )
                        AccountsList[i].enabled = true;

            } else if ( $(this).hasClass('disableAll') === true ) {
        ...
        $('div.setuptools.app.list > div.cell > div > input').keyup(function(e) {
        ...
        $('div.setuptools.accountControls > div').click(function() {
    ...
    setuptools.lightbox.build('accounts-manager', ' \
        <div class="setuptools accounts editor">\
            ...
            <div class="AccountsList"> \
                ...
                \
                <div class="controller empty saveNotice menuStyle cfleft" style="">&nbsp;</div> \
        ...
        if ( $(this).hasClass('saveAccounts') === true ) {
            ...
            if ( Object.keys(setuptools.data.accounts.accounts).length === 0 ) {
                ...
                if ( setuptools.app.config.save() === false ) {
                    ...
                    $('.controller.empty.saveNotice').removeClass('empty').addClass('notice').text('Reload Muledump to Apply').click(function() {
                        setTimeout(function() {
