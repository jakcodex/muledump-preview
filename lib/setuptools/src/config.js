...
setuptools.app.config.settings = function(highlight, section) {
    ...
    var sections = {
        ...
        settings: ['accountLoadDelay', 'accountsPerPage', 'alertNewVersion', 'automaticBackups', 'debugging', 'groups', 'longpress', 'maximumBackupCount', 'menuPosition', 'preventAutoDownload'],
    ...
    setuptools.lightbox.build('settings', ' \
        <h2>\
            <a href="#" class="setuptools link switchSettings muledump noclose">Muledump</a> | \
            <a href="#" class="setuptools link switchSettings settings noclose">SetupTools</a> | \
            <a href="#" class="setuptools link switchSettings assistants noclose">Assistants</a> | \
            <a href="#" class="setuptools link switchSettings system noclose">System</a> \
        </h2>\
        \
        <div class="setuptools containerSettings muledump' + ( (section !== 'muledump') ? ' hidden' : '' ) + '">\
            <h3>Muledump Settings</h3> \
    ...
    setuptools.lightbox.build('settings', ' \
            ...
            <div class="setuptools app config settings"><a href="#" class="setuptools app config settings action save">Save Settings</a><br></div>\
        </div>\
        \
        <div class="setuptools containerSettings settings' + ( (section !== 'settings') ? ' hidden' : '' ) + '"> \
            <h3>SetupTools Settings</h3> \
            <div class="setuptools app config settings"> \
    ...
    if ( setuptools.data.config.enabled === true ) {
        ...
        setuptools.lightbox.build('settings', ' \
                ...
                <div class="setuptools app config settings"> \
                    <div>Accounts Displayed per Page</div> \
        ...
        setuptools.lightbox.build('settings', ' \
                ...
                <div class="setuptools app config settings"> \
                    <div id="settings-groups">Groups Manager</div> \
                    <select name="groups" class="setting"> \
                        <option' + ( (setuptools.data.config.groups === false) ? ' selected' : '' ) + ' value="0">No</option> \
                        <option' + ( (setuptools.data.config.groups === true) ? ' selected' : '' ) + ' value="1">Yes</option> \
                    </select> \
                </div> \
                <div class="setuptools app config settings"> \
        ...
        setuptools.lightbox.build('settings', ' \
            ...
            <div class="setuptools containerSettings system' + ( (section !== 'system') ? ' hidden' : '' ) + '"> \
                <h3>System Data</h3> \
                <a href="#" class="setuptools app config settings action arrrrgbaaad noclose">Erase SetupTools</a> \
                <a href="#" class="setuptools app config settings action eraseCache noclose">Erase Cached Data</a> \
                <a href="#" class="setuptools app config settings action eraseAll noclose">Erase All Data</a> \
            </div> \
        ');
    ...
    $('.setuptools.app.config.settings.action.eraseCache').click(function() {

        setuptools.lightbox.build('settings-eraseCache-confirm', ' \
            This will erase all cached account data stored in your browser. \
            <br><br>Are you sure? \
            <br><br><a href="#" class="setuptools app eraseCache confirm">Yes, erase</a> or <a href="#" class="setuptools app cancelBadThing">No, cancel</a> \
        ');

        setuptools.lightbox.display('settings-eraseCache-confirm');

        $('.setuptools.app.eraseCache.confirm').click(function() {
            ...
            setuptools.lightbox.build('settings-eraseCache-complete', 'All cached data has been erased. This window reload in a few seconds.');
    ...
    $('.setuptools.app.config.settings.action.eraseAll').click(function() {

        setuptools.lightbox.build('settings-eraseAll-confirm', ' \
            This will erase all Muledump data stored in your browser. \
            <br><br>Are you sure? \
            <br><br><a href="#" class="setuptools app eraseAll confirm">Yes, erase</a> or <a href="#" class="setuptools app cancelBadThing">No, cancel</a> \
        ');

        setuptools.lightbox.display('settings-eraseAll-confirm');

        $('.setuptools.app.eraseAll.confirm').click(function() {
    ...
    $('.setuptools.app.config.settings.action.destroy').click(function() {
        ...
        $('.setuptools.app.config.settings select').each(function() {
            ...
            if ( ['enabled', 'preventAutoDownload', 'automaticBackups', 'debugging', 'groups'].indexOf(name) > -1 ) {
    ...
    $('.setuptools.app.config.settings.save').click(function() {
        ...
        if ( setuptools.data.config.enabled === true ) {
            ...
            settings.groups = ( $('.setuptools.app.config.settings select[name="groups"]').val() === '1' );
...
setuptools.app.config.backupAssistant = function() {
    ...
    if ( !lastBackup || (Date.now()-lastBackup >= (setuptools.data.config.backupAssistant*86400000)) ) {
        ...
        setuptools.lightbox.build('backup-assistant', ' \
            It is good practice to download a backup of your Muledump configuration from time-to-time.\
             \
            <br><br><a href="#" class="setuptools link downloadBackup">Download</a> latest backup or <a href="#" class="setuptools link cancel">Cancel</a> \
            \
            <br><br>You can disable the backup assistant in the <a href="#" class="setuptools app link settingsmanager">Settings Manager</a>. \
        ');

        setuptools.lightbox.settitle('backup-assistant', 'Backup Assistant');
        setuptools.lightbox.display('backup-assistant');

        setuptools.storage.write('backupAssistant', Date.now());

        $('.setuptools.link.downloadBackup').click(setuptools.app.backups.create);
...
setuptools.app.config.generateAccountsJS = function(accountConfig) {
    ...
    console.log(setuptools.app.config.convert(accountConfig, 0));
