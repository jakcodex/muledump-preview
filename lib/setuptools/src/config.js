...
setuptools.app.config.settings = function(highlight, section) {
    ...
    var sections = {
        ...
        settings: ['enabled', 'accountLoadDelay', 'accountsPerPage', 'automaticBackups', 'alertNewVersion', 'autoReloadDays', 'debugging', 'groupsMergeMode', 'loginOnlyTotals', 'longpress', 'maximumBackupCount', 'menuPosition', 'preventAutoDownload'],
    ...
    if ( setuptools.data.config.enabled === true ) {
        ...
        setuptools.lightbox.build('settings', ' \
                ...
                <div class="setuptools app config settings"> \
                    <div id="settings-mqAnimations">MuleQueue Animations</div> \
                    <select name="mqAnimations" class="setting"> \
                        <option' + ( (setuptools.data.config.mqAnimations === false) ? ' selected' : '' ) + ' value="0">Reduced</option> \
                        <option' + ( (setuptools.data.config.mqAnimations === true) ? ' selected' : '' ) + ' value="1">All</option> \
                    </select> \
                </div> \
                <div class="setuptools app config settings"> \
    ...
    $('.setuptools.link.config.settings.save').click(function() {
        ...
        if ( setuptools.data.config.enabled === true ) {
            ...
            settings.mqAnimations = ( $('.setuptools.app.config.settings select[name="mqAnimations"]').val() === '1' );
        }
        if ( setuptools.state.hosted === true ) {
