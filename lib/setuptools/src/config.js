...
setuptools.app.config.settings = function(highlight, section) {
    ...
    var sections = {
        muledump: ['rowlength', 'accountLoadDelay', 'errors', 'mulelogin', 'prices', 'nomasonry', 'testing', 'totalswidth', 'pagesearch'],
        settings: ['enabled', 'accountsPerPage', 'animations', 'autocomplete', 'automaticBackups', 'alertNewVersion', 'autoReloadDays', 'debugging', 'groupsMergeMode', 'loginOnlyTotals', 'longpress', 'maximumBackupCount', 'menuPosition', 'preventAutoDownload'],
        ...
        setuptools.lightbox.build('settings', ' \
                ...
                <div class="setuptools app config settings"> \
                    ...
                    <select name="lazySave" class="setting">\
                        ...
                        <option' + ( (setuptools.data.config.lazySave === 30000) ? ' selected' : '' ) + ' value="60000">30 seconds</option> \
                        <option' + ( (setuptools.data.config.lazySave === 60000) ? ' selected' : '' ) + ' value="100000">1 minute</option> \
                    </select> \
                </div> \
                <div class="setuptools app config settings"> \
    ...
    $('.setuptools.link.config.settings.action.destroy').click(function() {
        ...
        $('.setuptools.app.config.settings select').each(function() {
            ...
            if ( [
                    ...
                    'gaTotals', 'errors', 'autocomplete', 'mqDisplayIgn', 'muleMenu'
                ].indexOf(name) > -1 ) {
