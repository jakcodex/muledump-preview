...
setuptools.app.config.settings = function(highlight, section) {
    ...
    $('.setuptools.link.config.settings.save').click(function() {
        ...
        if ( Reroll === true ) {
            ...
            var ImportData = setuptools.storage.read('configuration');
            if ( setuptools.browser !== 'electron' ) ImportData = JSON.parse(ImportData);
...
setuptools.app.config.backup = function(backupProtected, auto) {
    ...
    var BackupStatus = setuptools.storage.write(BackupID, (
    ...
    return {
        ...
        BackupData: JSON.stringify(BackupData, null, 5),
        status: BackupStatus
    };
...
setuptools.app.config.save = function(message, lazy) {
    ...
    if ( setuptools.state.error === false && setuptools.config.devForcePoint !== 'config-save' ) {
        ...
        var saveResult = setuptools.storage.write('configuration', (
            setuptools.browser === 'electron'
        ) ?
            setuptools.data :
            JSON.stringify(setuptools.data, null, 5));
