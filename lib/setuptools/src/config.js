...
setuptools.app.config.settings = function(highlight, section) {
    ...
    var sections = {
        muledump: ['rowlength', 'accountLoadDelay', 'mulelogin', 'prices', 'nomasonry', 'testing'],
        settings: ['enabled', 'accountsPerPage', 'animations', 'automaticBackups', 'alertNewVersion', 'autoReloadDays', 'debugging', 'groupsMergeMode', 'loginOnlyTotals', 'longpress', 'maximumBackupCount', 'menuPosition', 'pagesearch', 'preventAutoDownload'],
    ...
    if ( setuptools.data.config.enabled === true ) {
        ...
        setuptools.lightbox.build('settings', ' \
                <div class="setuptools app config settings"> \
                    ...
                    <select name="animations" class="setting"> \
                        <option' + ( (setuptools.data.config.animations === false) ? ' selected' : '' ) + ' value="0">Reduced</option> \
                        <option' + ( (setuptools.data.config.animations === true) ? ' selected' : '' ) + ' value="1">All</option> \
                    </select> \
                </div> \
                <div class="setuptools app config settings"> \
        ...
        setuptools.lightbox.build('settings', ' \
                ...
                <!-- soon, 9.2 -- <div class="setuptools app config settings"> \
                    <div id="settings-loginOnlyTotals">Login Only Counts Totals</div> \
                    <select name="loginOnlyTotals" class="setting">\
                        <option' + ( (setuptools.data.config.loginOnlyTotals === true) ? ' selected' : '' ) + ' value="1">Yes</option> \
                        <option' + ( (setuptools.data.config.loginOnlyTotals === false) ? ' selected' : '' ) + ' value="0">No</option> \
                    </select> \
                </div>--> \
                <div class="setuptools app config settings"> \
        ...
        $('.setuptools.app.config.settings select').each(function() {
            ...
            if ( [
                ...
                //  these settings are integers in the config
                    ...
                    'backupAssistant', 'groupsMergeMode', 'autoReloadDays', 'pagesearch'
                ].indexOf(name) > -1 ) {
    ...
    $('.setuptools.link.config.settings.save').click(function() {
        ...
        if ( setuptools.data.config.enabled === true ) {
            ...
            settings.animations = ( $('.setuptools.app.config.settings select[name="animations"]').val() === '1' );
...
setuptools.app.config.save = function(message) {
