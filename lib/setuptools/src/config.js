...
setuptools.app.config.settings = function(highlight, section) {
    ...
    var sections = {
        muledump: ['accountLoadDelay', 'rowlength', 'errors', 'exportDefault', 'mulelogin', 'giftChestWidth', 'muleMenu', 'pagesearch', 'totalsExportWidth', 'totalswidth', 'nomasonry'],
        settings: ['enabled', 'accountsPerPage', 'animations', 'autocomplete', 'automaticBackups', 'autoReloadDays', 'alertNewVersion', 'badaccounts', 'compression', 'debugging', 'groupsMergeMode', 'hideHeaderText', 'lazySave', 'longpress', 'lowStorageSpace', 'maximumBackupCount', 'menuPosition', 'preventAutoDownload', 'timesync'],
    ...
    setuptools.lightbox.build('settings', ' \
        <div class="Settings-Menu fleft cboth flex-container" style="flex-wrap: 4; margin-bottom: 16px; overflow: hidden;">\
            <div class="setuptools link switchSettings muledump menuStyle cfleft noclose ml5 mb5">Muledump</div> \
            <div class="setuptools link switchSettings settings menuStyle fleft noclose ml5 mb5">SetupTools</div> \
            <div class="setuptools link switchSettings assistants menuStyle fleft noclose ml5 mb5">Assistants</div> \
            <div class="setuptools link switchSettings system menuStyle fleft noclose ml5 mb5">System</div> \
        </div>\
        \
    ...
    setuptools.lightbox.build('settings', ' \
        ...
        setuptools.lightbox.build('settings', ' \
            ...
            <div class="setuptools app config settings"' + disabled + '> \
                <div id="settings-errors">Mule Menu</div> \
    ...
    if ( setuptools.data.config.enabled === true ) {
        ...
        setuptools.lightbox.build('settings', ' \
                ...
                <div class="setuptools app config settings"> \
                    <div id="settings-badaccounts">Bad Accounts Reaction</div> \
                ...
                <div class="setuptools app config settings"> \
                    <div id="settings-macBindings">Mac OS Key Bindings</div> \
                    <select name="macBindings" class="setting">\
                        <option' + ( (setuptools.data.config.macBindings === true) ? ' selected' : '' ) + ' value="1">Yes</option> \
                        <option' + ( (setuptools.data.config.macBindings === false) ? ' selected' : '' ) + ' value="0">No</option> \
                    </select> \
                </div>\
                <div class="setuptools app config settings"> \
    ...
    setuptools.lightbox.build('settings', ' \
            <div class="fleft cboth">\
                ...
                <div class="setuptools link config settings action viewSystemsReport menuStyle notice cfleft mt5">View System Report</div> \
                <div class="setuptools link config settings action compression menuStyle notice cfleft mt5">Compression Utility</div> \
    ...
    $('.setuptools.link.compression').on('click.setuptools.link.compression', function() {
        ...
        setuptools.lightbox.goback('compression-utility', function() {
    ...
    $('.setuptools.link.config.settings.action.viewSystemsReport').on('click.setuptools.viewSystemsReport', setuptools.app.viewSystemsReport);
    ...
    $('.setuptools.link.config.settings.action.destroy').click(function() {
        ...
        $('.setuptools.app.config.settings select').each(function() {
            ...
            if ( [
                    ...
                    'compression', 'timesync', 'badaccounts'
                ].indexOf(name) > -1 ) {
