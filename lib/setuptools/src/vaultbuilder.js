...
setuptools.app.muledump.vaultbuilder.menu.import = function(mode) {
    ...
    if ( mode === 'import' ) html += ' \
        ...
        <div class="flex-container mb5">\
            <input type="text" name="saveAsName" placeholder="Layout Name" class="setuptools flex-container mr5 noFlexAutoWidth" style="height: 17px; flex-grow: 2; width: 25%;">\
            <input type="text" name="saveAsId" placeholder="Layout ID Number" class="setuptools flex-container noFlexAutoWidth" style="height: 17px; flex-grow: 2; width: 25%;">\
        </div>\
        \
        <div class="flex-container">\
    ...
    options.push({
        ...
        binding: function() {
            ...
            $('input[type="file"]').on('change.input.file', function(e) {
                ...
                label.text(this.files.length + " file" + ( (this.files.length !== 1) ? 's' : '' ) + ' selected');
                ...
                Object.keys(this.files).forEach(function(index) {
                    ...
                    reader.onload = function () {
                        ...
                        if (reader.error) {
                            ...
                            label.test('Failed to parse selected file(s)');

                        } else {

                            var zip = new JSZip();
                            zip.loadAsync(reader.result).then(function (file) {
                                console.log();
                                zip.file(file).async("string").then(function(string) {
                                    console.log(string);
                                });
                            });
