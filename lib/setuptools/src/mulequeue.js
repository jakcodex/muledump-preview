...
setuptools.app.mulequeue.task.queue = function(guids, config) {
    ...
    for ( var i = 0; i < guids.length; i++ ) {
        ...
        config.reloader.empty().append(setuptools.app.mulequeue.spinner.running()).attr('title', 'Task is waiting in MuleQueue');
...
setuptools.app.mulequeue.task.start = function(guid, callback) {
    ...
    setuptools.app.mulequeue.tasks[guid].promise = setuptools.app.mulequeue.task.createPromise(guid).then(function(response) {
        ...
        if (response.state === 'finished' && task.state === 'running' ) {
            ...
            if ( typeof setuptools.lightbox.menu.paginate.state.QueueList === 'object' ) setuptools.lightbox.menu.paginate.state.QueueList.Modifiers.callbacks.update();
...
setuptools.app.mulequeue.task.pause = function(guids, callback) {
    ...
    for ( var i = 0; i < pauseList.length; i++ ) {
        ...
        if ( task.state === 'queue' ) {
            ...
            task.reloader.empty().append(setuptools.app.mulequeue.spinner.running('pause')).attr('title', 'Task paused in MuleQueue');
...
setuptools.app.mulequeue.task.resume = function(guids, callback) {
    ...
    for ( var i = 0; i < pauseList.length; i++ ) {
        ...
        if ( task.state === 'queue' ) {
            ...
            task.reloader.empty().append(setuptools.app.mulequeue.spinner.running('queue')).attr('title', 'Task is waiting in MuleQueue');
...
setuptools.app.mulequeue.ui.manager = function(guid) {
    ...
    function DrawButtons() {
        ...
        if ( setuptools.app.mulequeue.state.running === false ) {
            ...
            html += ' \
                <div class="setuptools menuStyle buttonStyle positive mulequeue main menu reload noselect" title="Reload All Accounts" style="float: right; font-weight: normal;">&#9654;</div> \
            ';
    ...
    function createRow(listPos) {
        ...
        return ' \
            <div class="flex-container queuetask noselect" style="justify-content: flex-start;" data-listPos="' + listPos + '"> \
                <div style="flex-basis: 30px; height: 24px; justify-content: center; align-items: center;" class="flex-container textCenter" title="Queue Position: ' + (listPos+1) + '">' + setuptools.app.mulequeue.spinner.running(undefined, true) + '</div> \
                <div style="flex-basis: 400px; justify-content: flex-start; border: 0;" class="menuStyle buttonStyle neutral textLeft w100 select mr0">' + QueueList[listPos] + '</div> \
            </div>\
        ';
    ...
    function QueueManagerUpdate(index) {

        if ( !index && typeof setuptools.app.mulequeue.state.topListPos === 'number' ) index = setuptools.app.mulequeue.state.topListPos;
        if ( ['string', 'number', 'undefined'].indexOf(typeof index) === -1 ) return;
        var data = setuptools.lightbox.menu.paginate.state.QueueList;
        if ( ['string', 'number'].indexOf(typeof index) > -1 ) setuptools.lightbox.menu.paginate.findPage(index, 'QueueList');
        ...
        $('div.QueueList.list').html(' \
            ...
            <div class="setuptools app list w100 cboth">\
                ' + ListQueue(undefined, data.currentPage) + ' \
            </div> \
    ...
    function FindTopListPos() {

        var data = setuptools.lightbox.menu.paginate.state.QueueList;
        setuptools.app.mulequeue.state.topListPos = data.currentPage*setuptools.data.config.accountsPerPage;
    ...
    var QueueManagerPaginate = setuptools.lightbox.menu.paginate.create(
        ...
        QueueManagerContext,
        {
            pageButtons: {callback: DrawButtons},
            callbacks: {
