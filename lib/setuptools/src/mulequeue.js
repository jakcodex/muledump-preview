...
setuptools.app.mulequeue.ui.drawButtons = function() {
    ...
    if ( setuptools.app.mulequeue.state.running === false ) {
        ...
        html += ' \
            <div class="setuptools menuStyle buttonStyle positive mulequeue main menu reload noselect" title="Reload All Accounts" style="float: right; padding: 0; display: flex; justify-content: center; align-items: center;"><div style="font-size: 15px; font-weight: normal;">&#9654;</div></div> \
                    ';
...
setuptools.app.mulequeue.ui.manager = function(guid) {
    ...
    function createRow(listPos) {
        ...
        var NextRun = setuptools.data.config.accountLoadDelay-LastRunDiff;
        var WaitTime = {
            estimation: (((listPos+1)*(setuptools.data.config.accountLoadDelay+TimePerTask))-LastRunDiff)
        };
