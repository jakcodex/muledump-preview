...
setuptools.app.mulequeue = {
    ...
    state: {
        ...
        timePerTaskAverage: 2000,               //  average time it takes to run each task in miliseconds
    },
...
setuptools.app.mulequeue.task.rateLimit = function(soft) {
    ...
    function RateLimitTimer() {
        ...
        if (RateLimitRemaining <= 0) {
            ...
            if ( setuptools.data.config.animations === true ) $('#fix').removeClass('rateLimited nopulse');
            ...
            $('#stickynotice').html("<div>Warning: Your account is rate limited by Deca. You must wait " + RateLimitTime + " before you can reload your account.</div>");
    ...
    $('div.mulequeue.progress > div.bar').addClass('rateLimited');
    if ( setuptools.data.config.animations === true ) $('#fix').addClass('rateLimited nopulse');
...
setuptools.app.mulequeue.task.reorder = function(guids, position) {
    ...
    for ( var i = 0; i < guids.length; i++ ) {
        ...
        if (taskIndex !== taskPosition) {
            ...
            window.techlog('MuleQueue/Start pushing ' + guids[i] + ' to position ' + taskPosition, 'force');
...
setuptools.app.mulequeue.ui.manager = function(guid) {
    ...
    function createRow(listPos) {
        ...
        var TimePerTask = Math.round(setuptools.app.mulequeue.state.timePerTaskAverage/1000)+1;
        ...
        return ' \
            <div class="flex-container queuetask noselect" style="justify-content: flex-start;" data-listPos="' + listPos + '" data-guid="' + QueueList[listPos] + '"> \
                ...
                <div style="flex-basis: 539px; justify-content: space-between; border: 0;" class="menuStyle buttonStyle neutral textLeft w100 select mr0">\
                    <div style="flex-basis: 439px; overflow: hidden; word-wrap: break-word;" class="select">' + QueueList[listPos] + '</div> \
                ...
                <div style="flex-basis: 30px; height: 24px; justify-content: flex-end;" class="flex-container">\
                    <div style="flex-basis: 25px;" class="flex-container setuptools menuStyle buttonStyle link taskMenu noclose mr0">&#8801;</div> \
                </div>\
            </div>\
        ';
...
$('div.setuptools.link.taskMenu').click(function() {
    ...
    setuptools.lightbox.build('mulequeue-manager', ' \
        <div class="mulequeue progress w100 flex-container ' + AReduced + '" style="position: relative; border: 2px solid #000; height: 34px; margin: 10px 0; justify-content: center;">\
            <div class="percentage" style="z-index: 5000; text-shadow: 1px 1px #000000; background: rgba(0, 0, 0, 0.15); padding: 2px;">No Tasks</div> \
            <div class="bar ' + rateLimited + ' nopulse" style="position: absolute; left: 0; top: 0; height: 30px; width: 0;">\
