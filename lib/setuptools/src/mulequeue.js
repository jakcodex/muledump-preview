...
setuptools.app.mulequeue.task.saveConfig = function() {
...
//  load mulequeue cache data and import it if necessary
setuptools.app.mulequeue.task.loadConfig = function() {

    //  import any mulequeue cache data
    var json = setuptools.storage.read('mulequeue:config');
    var Config;
    try {
...
setuptools.app.mulequeue.task.rateLimit = function(soft) {
    ...
    function RateLimitTimer() {
        ...
        if (RateLimitRemaining <= 0) {
            ...
            if ( setuptools.data.config.animations === true ) $('#stickynotice').removeClass('rateLimited nopulse');
            ...
            for ( var task in setuptools.app.mulequeue.tasks ) {
                ...
                if ( setuptools.app.mulequeue.tasks.hasOwnProperty(task) ) {
                    ...
                    if ( setuptools.app.mulequeue.tasks[task].state === 'queue' && setuptools.data.config.animations === true ) {
            ...
            $('#stickynotice').html('<div>Warning: Your account is <a href="' + setuptools.config.ratelimitHelp + '" class="rateLimited nopulse" target="_blank">rate limited</a> by Deca. You must wait ' + RateLimitTime + ' before you can reload your account.</div>');
            $('div.mulequeue.progress > div.percentage').html('You are <a href="' + setuptools.config.ratelimitHelp + '" class="rateLimited nopulse" target="_blank">rate limited</a> and must wait ' + RateLimitTime + ' before continuing.');
    ...
    if ( setuptools.app.mulequeue.queue.length > 0 ) {
        for (var index = (setuptools.app.mulequeue.state.busy - 1); index >= 0; index--) {
            ...
            if ( setuptools.data.config.animations === true ) {
    ...
    if ( setuptools.data.config.animations === true ) $('#stickynotice').addClass('rateLimited nopulse');
...
setuptools.app.mulequeue.task.queue = function(guids, config) {
    ...
    for ( var i = 0; i < guids.length; i++ ) {
        ...
        if ( setuptools.data.config.animations === true ) config.reloader.empty().append(setuptools.app.mulequeue.spinner.running()).attr('title', 'Task is waiting in MuleQueue');
...
setuptools.app.mulequeue.task.start = function(guid, callback) {
    ...
    if ( setuptools.data.config.animations === true ) setuptools.app.mulequeue.tasks[guid].reloader.empty().append(setuptools.app.mulequeue.spinner.running('running')).attr('title', 'Mule data request running...');
    ...
    setuptools.app.mulequeue.tasks[guid].promise = setuptools.app.mulequeue.task.createPromise(guid).then(function(response) {
        ...
        if (response.state === 'finished' && task.state === 'running' ) {
            ...
            if ( response.status === 'error' ) {
                ...
                if ( setuptools.data.config.animations === true ) task.reloader.empty().append(setuptools.app.mulequeue.spinner.running('error')).attr('title', 'Error loading account: ' + response.errorMessage);
...
setuptools.app.mulequeue.task.pause = function(callback) {
    ...
    for ( var i = 0; i < pauseList.length; i++ ) {
        ...
        if ( task.state === 'queue' ) {
            ...
            if ( setuptools.data.config.animations === true ) task.reloader.empty().append(setuptools.app.mulequeue.spinner.running('pause')).attr('title', 'Task paused in MuleQueue');
...
setuptools.app.mulequeue.task.resume = function(guids, callback) {
    ...
    for ( var i = 0; i < pauseList.length; i++ ) {
        ...
        if ( task.state === 'queue' ) {
            ...
            if ( setuptools.data.config.animations === true ) task.reloader.empty().append(setuptools.app.mulequeue.spinner.running('queue')).attr('title', 'Task is waiting in MuleQueue');
...
setuptools.app.mulequeue.ui.drawButtons = function() {
    ...
    html += ' \
        <div class="fleft flex-container nohover mulequeue queueCount">' + setuptools.app.mulequeue.queue.length + ' Tasks in Queue</div>\
    ';
...
setuptools.app.mulequeue.ui.manager = function(guid) {
    ...
    function createRow(listPos) {
        ...
        return ' \
            <div class="flex-container queuetask noselect" style="justify-content: flex-start;" data-listPos="' + listPos + '" data-guid="' + QueueList[listPos] + '"> \
                <div class="flex-container textCenter" title="Queue Position: ' + (listPos+1) + '">' + setuptools.app.mulequeue.spinner.running(undefined, true) + '</div> \
                <div class="menuStyle buttonStyle neutral textLeft w100 select mr0">\
                    <div class="select">' + QueueList[listPos] + '</div> \
    ...
    function QueueManagerContext() {
        ...
        $('div.setuptools.link.taskMenu').click(function() {
            ...
            if ( setuptools.data.config.mulelogin === 1 ) options.push({
                ...
                href: setuptools.app.muledump.mulelink(QueueList[listPos])
            });
            ...
            options.push({
                ...
                attributes: {
                    'data-clipboard-text': QueueList[listPos]
                }
            });

            if ( setuptools.app.config.determineFormat(setuptools.data.accounts) === 1 && setuptools.data.accounts.accounts[QueueList[listPos]].ign ) options.push({
                class: 'copySelection',
                name: 'Copy IGN',
                attributes: {
                    'data-clipboard-text': setuptools.data.accounts.accounts[QueueList[listPos]].ign
                }
            });

            setuptools.lightbox.menu.context.create('taskMenu', false, $(this), options, $(this));
    ...
    var AReduced = '';
    var rateLimited = '';
    if ( setuptools.data.config.animations === false ) AReduced = 'AReduced';
