...
setuptools.app.mulequeue.task.saveQueueCache = function() {

    var QueueCache = {date: Date.now(), data: []};
    for ( var guid in setuptools.app.mulequeue.tasks )
        if (setuptools.app.mulequeue.tasks.hasOwnProperty(guid))
            if ( setuptools.app.mulequeue.tasks[guid].action === 'reload' )
                QueueCache.data.push(guid);

    setuptools.storage.write('mulequeue:queue', QueueCache);

};

//  load mulequeue cache data and import it if necessary
setuptools.app.mulequeue.task.loadQueueCache = function() {

    //  import any mulequeue cache data
    var json = setuptools.storage.read('mulequeue:queue');
    var QueueCache;
    try {
        QueueCache = JSON.parse(json);
    } catch(e) {}
    if (
        typeof QueueCache === 'object' &&
        QueueCache !== null &&
        (Date.now()-QueueCache.date) < setuptools.config.mqStaleCache
    ) setuptools.app.mulequeue.task.reload(QueueCache.data);
...
setuptools.app.mulequeue.task.start = function(guid, callback) {
    ...
    setuptools.app.mulequeue.tasks[guid].promise = setuptools.app.mulequeue.task.createPromise(guid).then(function(response) {
        ...
        if (response.state === 'finished' && task.state === 'running' ) {
            ...
            setuptools.app.mulequeue.state.timePerTaskAverage = (setuptools.app.mulequeue.state.timePerTaskAverage+task.runTime)/2;
            ...
            } else setuptools.app.mulequeue.task.saveQueueCache();
...
setuptools.app.mulequeue.task.cancel = function(guids, callback) {
    ...
    setuptools.app.mulequeue.task.saveQueueCache();
