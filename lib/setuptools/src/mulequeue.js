...
setuptools.app.mulequeue.task.background = function() {
    ...
    if (
        ...
        if (
            ...
            window.techlog('MuleQueue/TaskWait - ' + Math.ceil(window.accountLoadDelay-lastRunDifference) + ' seconds until next cycle');
            return;
...
setuptools.app.mulequeue.task.loadConfig = function() {
    ...
    if (
        ...
        if ( (Date.now()-Config.date) < setuptools.config.mqStaleCache && Array.isArray(Config.queue) && Config.queue.length > 0 ) {
            ...
            if ( setuptools.state.loaded === true ) Config.queue.filter(function(item) {
                return (
    ...
    setuptools.app.mulequeue.tasks[guid].promise = setuptools.app.mulequeue.task.createPromise(guid).then(function(response) {
        ...
        if (response.state === 'finished' && task.state === 'running' ) {
            ...
            setuptools.app.mulequeue.task.history({
                ...
                state: task.state,
                guid: response.guid,
                ign: ( (setuptools.app.config.determineFormat(setuptools.data.accounts) === 1) ? setuptools.data.accounts.accounts[response.guid].ign : undefined ),
...
setuptools.app.mulequeue.task.cancel = function(guids, callback) {
    ...
    for ( var i = (guids.length-1); i >= 0; i-- ) {
        ...
        if ( ['queue', 'error'].indexOf(task.state) > -1 ) {
            ...
            setuptools.app.mulequeue.tasks[guid].reloader.empty().text('\u21bb').attr('title', 'last updated: ' + ( (typeof mules[guid].data === 'object') ?
...
setuptools.app.mulequeue.ui.history = function() {
    ...
    var createRow = function(listPos) {
        ...
        var record = {
            ...
            guid: ( (
                ...
                setuptools.app.mulequeue.state.displayIgn === true &&
                typeof setuptools.data.accounts.accounts[QueueRecord.guid].ign === 'string'
            ) ? setuptools.data.accounts.accounts[QueueRecord.guid].ign : QueueRecord.guid ),
    ...
    var QueueHistoryContext = function() {
        ...
        $('div.mulequeue.link.nameswap').unbind('click').click(function() {
            setuptools.app.mulequeue.state.displayIgn = !(setuptools.app.mulequeue.state.displayIgn);
            QueueHistoryUpdate();
        });
    ...
    var pageButtonsHtml = ' \
        ...
        <div class="flex-container nohover mulequeue link nameswap menuStyle buttonStyle noFlexAutoWidth mt5" title="Switch Account Name Display"><div style="font-weight: normal; overflow: hidden;">&#9788;</div></div> \
