...
setuptools.app.ga = function(command, action, value1, value2, value3, value4, value5) {
    ...
    if ( command === 'init' ) {
        ...
        if ( setuptools.state.loaded === true && (setuptools.data.userid === false || !setuptools.data.userid.match(setuptools.config.regex.gaUserId)) ) {
            ...
            setuptools.data.userid = setuptools.seasalt.hash.sha256(Date.now());
        ...
        if ( setuptools.data.config.debugging === false ) {

            delete timing.scriptInit;
            delete timing.mdInit;
...
setuptools.app.checkperf = function() {
    ...
    if (
        ...
        console.log('perf', setuptools.data.acknowledge.assistants.performance, 'this is !== true');
...
setuptools.app.uaTimer = function(category, key, action, uid) {

    if ( ['start', 'stop', 'cancel'].indexOf(action) === -1 ) return;
    var nameKey = category + '-' + key;
    if ( typeof uid === 'string' ) nameKey += '-' + uid;
    ...
    //  start a timer
    if ( action === 'start' ) timing[nameKey] = new JTimer({
        name: nameKey,
