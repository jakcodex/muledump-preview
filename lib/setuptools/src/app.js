...
setuptools.app.index = function(config) {
    ...
    if ( setuptools.app.checknew() === true ) {
        ...
        setuptools.lightbox.build('main', ' \
            ...
            <div class="flex-container" style="justify-content: space-evenly; flex-wrap: wrap;">\
                ...
                <div class="setuptools link config ocl menuStyle menuSmall mt10">One Click Login</div>\
                <div class="setuptools link config devtools menuStyle menuSmall notice mt10">Dev Tools</div>\
            </div>\
        ');
    ...
    $('.setuptools.config.devtools').on('click.setuptools.devtools', setuptools.app.devtools);

};

setuptools.app.devtools = function() {
    ...
    $('div.setuptools.link.systemsreport').on('click.devtools.vaultbuilder', setuptools.app.viewSystemsReport);
    $('div.setuptools.link.configviewer').on('click.devtools.configviewer', setuptools.app.configViewer);
    $('div.setuptools.link.consoleviewer').on('click.devtools.consoleviewer', setuptools.app.consoleViewer);
    setuptools.lightbox.override('settings-viewSystemsReport', 'goback', setuptools.app.devtools);

};

setuptools.app.consoleViewer = function() {

    var data = JSON.parse(setuptools.storage.read('console'));
    if ( !Array.isArray(data) ) data = [];

    setuptools.lightbox.build('devtools-consoleViewer', ' \
        <div class="flex-container m5 p5" style="justify-content: flex-end;">\
    ...
    for ( var i = data.length-1; i >= 0; i-- ) {
        ...
        setuptools.lightbox.build('devtools-consoleViewer', ' \
            <div class="flex-container muledump consoleviewer m5 p5">\
                <div style="width: 14%;">' + record[0] + '</div>\
                <div style="width: 14%;">' + record[1] + '</div>\
                <div style="width: 8%;">' + record[2] + '</div>\
                <div style="width: 64%;">' + record[3] + '</div>\
            </div>\
        ');
    ...
    setuptools.lightbox.goback('devtools-consoleViewer', setuptools.app.devtools);
    setuptools.lightbox.display('devtools-consoleViewer', {variant: 'fl-ConsoleViewer select'});

    $('div.setuptools.link.consoleviewer.reload').on('click.consoleviewer.reload', setuptools.app.consoleViewer);

};

setuptools.app.configViewer = function(mode) {
    ...
    setuptools.lightbox.goback('devtools-configViewer', setuptools.app.devtools);
...
setuptools.app.recordConsole = function() {
    ...
    setuptools.tmp.recordConsoleTask = setInterval(function() {
        ...
        ) consoleHistory.splice(0, consoleHistory.length-setuptools.data.config.recordConsole);
...
setuptools.app.techlog = function(msg, type) {
    ...
    if ( (setuptools.data.config.debugging === true && ['info','norecord'].indexOf(type) > -1) || ['force'].indexOf(type) > -1 ) console.log(ts, '-', msg);

    if ( ['norecord', 'api'].indexOf(type) === -1 && typeof setuptools.data.config.recordConsole === 'number' && setuptools.data.config.recordConsole !== 0 ) {
