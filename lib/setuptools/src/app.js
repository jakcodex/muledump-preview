...
setuptools.app.ga = function(command, action, value1, value2, value3, value4, value5) {
    ...
    if ( command === 'init' ) {
        ...
        analytics('send', 'event', {
            ...
            eventLabel: ( ( setuptools.browser === 'electron' ) ? 'app-' : '' ) + ( setuptools.state.hosted === true ) ? 'online' : 'local'
        });
...
setuptools.app.recordConsole = function() {
    ...
    setuptools.tmp.recordConsoleTask = setInterval(function() {
        ...
        var consoleHistory = setuptools.storage.read('console');
        if ( setuptools.browser !== 'electron' ) consoleHistory = JSON.parse(consoleHistory);
