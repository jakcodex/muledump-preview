...
setuptools.app.assistants.account = function(url, extraopts, callback) {
    ...
    if ( extraopts.type === 'ageVerify' && setuptools.state.assistant[extraopts.type] === false ) {
        ...
        setuptools.lightbox.build('muledump-account-assistant', ' \
				...
				<br><br>Please proceed to: <a href="' + url + '" target="_blank" class="setuptools app link popupComplete">ROTMG Account TOS Verification</a> \
				<br><br>You can disable the account assistant in the <a href="#" class="setuptools app link settingsmanager">Settings Manager</a>. \
			');
    ...
    setuptools.lightbox.display('muledump-account-assistant', {variant: 'setuptools-medium', afterClose: ( ( typeof callback === 'function' ) ? callback : function() {
...
setuptools.app.assistants.cors = function(force) {
    ...
    setuptools.lightbox.build('muledump-cors', ' \
        ...
        ( ( ['chrome','opera'].indexOf(setuptools.browser) === -1 ) ? ' \
...
setuptools.app.assistants.jakcodexcors = function() {
    ...
    setuptools.lightbox.build('jakcodexcors-extension-notice', " \
        Jakcodex/Muledump is pleased to introduce a new CORS extension now available in the Chrome Web Store!\
        <br><br>\
        <div class='flex-container'>\
        ...
        <br>Since adding Usage Analytics we now know that 65% of daily users experience some form of CORS errors. Many users experience it multiple times per day. This is simply unacceptable.\
        <br><br>This new extension is always on and requires no configuration. Remove the old extension, install the new extension, and you're ready to go. \
