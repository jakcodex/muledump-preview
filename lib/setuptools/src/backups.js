...
setuptools.app.backups.index = function() {
    ...
    if ( BackupList.length === 0 ) {
        ...
        for (i = BackupList.length - 1; i >= 0; i--) {
            ...
            if (BackupList.hasOwnProperty(i)) {
                ...
                var found = false;
                var BackupData = setuptools.storage.read(BackupList[i][1]);
                if ( setuptools.browser !== 'electron' ) BackupData = JSON.parse(BackupData);
    ...
    function updateProtection() {
        ...
        var SelectedBackupData = setuptools.storage.read(SelectedBackupID);
        if ( setuptools.browser !== 'electron' ) SelectedBackupData = JSON.parse(SelectedBackupData);
...
setuptools.app.backups.restore = function(options) {
    ...
    ) options.backupId = ( setuptools.browser === 'electron' ) ? options.archive.setuptools : JSON.stringify(options.archive.setuptools);
    ...
    if ( options.method === 'local' || options.method === 'upload' ) {
        ...
        if ( BackupFile ) {
            ...
            var BackupData;
            try {
                BackupData = ( setuptools.browser === 'electron' ) ? BackupFile : JSON.parse(BackupFile);
            } catch(e){}
        ...
        if ( typeof options.archive === 'object' ) {
            ...
            if ( options.archive.accounts && Object.keys(options.archive.accounts).length > 0 ) {
                ...
                Object.keys(options.archive.accounts).filter(function(guid) {
                    ...
                    var data = options.archive.accounts[guid];
                    bytes += JSON.stringify(data).length;
                    if ( setuptools.browser !== 'electron' ) data = JSON.stringify(data);
...
setuptools.app.backups.protect = function(BackupID, BackupName, BackupProtected) {
    ...
    if ( typeof BackupData === 'object' && BackupData.meta ) {
        ...
        if ( BackupData.meta.protected === BackupProtected ) {
            ...
            //  save changes
            var write = setuptools.storage.write(BackupID, BackupData, true);
            if ( write === true ) {
...
setuptools.app.backups.renameHtml = function(page, BackupID, BackupDataJSON, callback, noclose) {
    ...
    return function() {
        ...
        var Process = function(self) {
            ...
            setuptools.storage.write(BackupID, (
                setuptools.browser === 'electron'
            ) ?
                BackupData :
                JSON.stringify(BackupData, null, 5));
...
setuptools.app.backups.archive.create = function(guids, callback) {
    ...
    if ( typeof guids === 'undefined' ) guids = Object.keys(setuptools.data.accounts && setuptools.data.accounts.accounts || window.accounts);
    ...
    guids.filter(function(guid) {
        ...
        var accountdata = setuptools.storage.read('muledump:' + guid, true);
        if ( setuptools.browser !== 'electron' ) accountdata = JSON.parse(accountdata);
        var data = JSON.stringify(accountdata, null, 5);
...
setuptools.app.backups.cleanup = function() {
    ...
    for ( i = backups.length-1; i >= 0; i-- )
        if ( backups.hasOwnProperty(i) ) {
            ...
            var BackupData = setuptools.storage.read(backups[i][1]);
            if ( setuptools.browser !== 'electron' ) BackupData = JSON.parse(BackupData);
