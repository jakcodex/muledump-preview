...
class Muledump_MouseDirection {
    ...
    constructor(config) {
        ...
        this.runtimeId = (new SeaSalt_Hashing(Date.now().toString(), 'sha256').toString()).substr(56, 64);
        ...
        $(this.targetName).on('mousedown.' + this.namespace, function(e) {
            ...
            //  move the cell with the mouse
                ...
                .on('mousemove.' + parent.namespace, function(e) {
                    ...
                    //  but we will insert after if we're not approaching the target div
                    if ( parent.MouseDirection.approach.h === false ) {
                        parent.indexModifier++;
                    }
                    //  or insert after if we're approaching the target div vertically and on the right side
                    else if ( typeof parent.MouseDirection.travel.h !== 'boolean' && typeof parent.MouseDirection.travel.v === 'boolean' ) {
                        parent.indexModifier++;
                    }

                    else if ( parent.MouseDirection.travel.v === false && parent.MouseDirection.rel.v === true ) {
                        parent.indexModifier = 0;
                    }

                    else if ( parent.MouseDirection.travel.v === true && parent.MouseDirection.rel.v === true ) {
                        parent.indexModifier++;
                    }
