    ...
    import(tracker, excluded) {
        ...
        if ( tracker instanceof Muledump_TotalsCounter &&
            ...
            if ( this.sources.indexOf(tracker.guid) > -1 ) {
                ...
                if ( new SeaSalt_Hashing(JSON.stringify(this.sourceData[tracker.guid].meta), 'sha256', 'hex').toString() === new SeaSalt_Hashing(JSON.stringify(tracker.data.meta), 'sha256', 'hex').toString() ) {
