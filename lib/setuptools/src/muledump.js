...
setuptools.app.muledump.mulelink = function(guid) {
    ...
    //  does not support steam
    if ( guid.indexOf('steamworks:') === 0 ) return;
...
setuptools.app.muledump.realmeye.link = function(itemNumber, link, extra) {
    ...
    //  determine itemName
    if ( typeof itemNumber === 'number' ) itemName = items[itemNumber][0];
    ...
    if ( link === 'wiki' ) {
        ...
        if ( items[itemNumber][8] === true ) return;
        link = 'offers-to/sell';
        itemName = itemNumber;
    } else if ( link === 'buy' ) {
        if ( items[itemNumber][8] === true ) return;
    ...
    if ( typeof extra === 'number' ) extra = '/' + extra;
