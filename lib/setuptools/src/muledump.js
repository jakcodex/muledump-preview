    ...
    $('.itemSlotTypeSorting.cell')
        ...
        .on('dblclick.muledump.totals.itemGroupSorting', function(e) {
            ...
            if ( slotSubOrder[vst].length > 0 ) slotSubOrder[vst].filter(function(itemid) {
                ...
                var selected = ( setuptools.app.muledump.totals.config.getKey('itemFilter').indexOf(+itemid) > -1 ) ? '' : 'selected';
                setuptools.lightbox.build('totalsmenu-itemSorting', '<div class="item subsorting cell ' + selected + ' noselect" data-itemId="' + itemid + '" style="background-position: -' + item[3] + 'px -' + item[4] + 'px;"></div>');
...
setuptools.app.muledump.totals.menu.main = function(track) {
    ...
    if ( setuptools.data.options.totals === true ) {
        ...
        },
        {
            ...
            name: ( (setuptools.data.options.nonsbfilter === true) ? 'Disable' : 'Show Only' ) + ' Non-Soulbound',
            callback: setuptools.app.muledump.toggleoption,
