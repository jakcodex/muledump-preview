...
setuptools.app.muledump.exporter.text = function(type) {
    ...
    e.txt = e.txt.join('\n');
    e.csv = e.csv.join('\n');
    e.json = JSON.stringify(e.json, null, '\t').replace(/(\t)/g, '&nbsp;&nbsp;&nbsp;&nbsp;');
    return e[type];

};

setuptools.app.muledump.exporter.pastebin = function(type, data, callback) {

    if ( typeof callback !== 'function' ) callback = function(data, err, errDesc) {

        setuptools.app.techlog('Muledump/Exporter Pastebin API response: ' + JSON.stringify(data));

    };

    var format = 'text';
    if ( type === 'json' ) format = 'json';
    $.ajax('https://pastebin.com/api/api_post.php', {
        method: 'POST',
        dataType: 'json',
        crossDomain: true,
        data: {
            api_dev_key: setuptools.data.exporter.remote.pastebin,
            api_option: 'paste',
            api_paste_code: data,
...
setuptools.app.muledump.exporter.ui.main = function(canvas, type) {
    ...
    setuptools.lightbox.build('exporter-main', '\
        <div class="flex-container exporterMenu">\
            ...
            <div class="setuptools link canvas totals export remote imgur menuStyle menuSmall notice textCenter noclose mr0">Upload to Imgur</div>\
    ...
    mode.on('change.totals.export.mode', function() {
        ...
        if ( $(this).val() === 'image' ) {
            ...
            remote.removeClass('pastebin').addClass('imgur').text('Upload to Imgur');

        } else {

            remote.removeClass('imgur').addClass('pastebin').text('Upload to Pastebin');
