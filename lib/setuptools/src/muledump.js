...
setuptools.app.muledump.skinWardrobe = function(guid, tex1, tex2) {
    ...
    Object.filter(setuptools.tmp.skins[guid], function(classid, data) {
        ...
        data.skins.filter(function(skinid) {
            ...
            console.log(classid, skinid, tex1, tex2);
...
setuptools.app.muledump.ocl.manager = function(profile, advanced, hardreset) {
    ...
    setuptools.lightbox.build('ocl-manager', ' \
        ...
        <div class="ocl manager">\
            <div class="ocl configuration">\
                ...
                <div data-ocl="title" style="' + ( (data.params.mode === 'browser' || advanced === false) ? 'display: none;' : '' ) + '">\
                ...
                <div data-ocl="ign" style="' + ( (data.params.mode === 'browser' || advanced === false) ? 'display: none;' : '' ) + '">\
    ...
    var help = {
        ...
        path: ['<code>Flash Projector:</code> Path to Flash Projector exe (e.g. C:\\flashplayer_16_sa.exe)', '<code>Browser:</code> URL to launch the game'],
    ...
    $('div.setuptools.link.ocl.advanced').on('click.ocl.advanced', function() {
        ...
        if ( $(this).attr('data-state') === 'false' ) {
            ...
            if ( mode === 1 ) $('div[data-ocl="title"]').slideDown();
            if ( mode === 1 ) $('div[data-ocl="ign"]').slideDown();
            return;
        ...
        if ( mode === 1 ) $('div[data-ocl="title"]').slideUp();
        if ( mode === 1 ) $('div[data-ocl="ign"]').slideUp();
        if ( mode === 1 ) $('div[data-help="ocl title"]').slideUp(function() { this.remove() });
        if ( mode === 1 ) $('div[data-help="ocl ign"]').slideUp(function() { this.remove() });
        if ( mode === 1 ) $('div[data-ocl="title"] strong.help').html('?');
        if ( mode === 1 ) $('div[data-ocl="ign"] strong.help').html('?');
...
setuptools.app.muledump.ocl.install = function() {
    ...
    setuptools.lightbox.build('ocl-install', ' \
        <div id="drawhelpBox">\
            ...
            ' : '' ) + '\
            \
            <br>The latest version of One Click Login is available on Github: <a href="' + setuptools.config.url + '/lib/mulelogin.au3" target="_blank" download="mulelogin.au3">' + setuptools.config.url + '/lib/mulelogin.au3</a> \
            \
...
setuptools.app.muledump.ocl.record = function(guid) {
    ...
    setuptools.app.ga('send', 'event', {
        eventCategory: 'ocl',
        eventAction: 'mode',
        eventLabel: profile.params.mode
    });
