...
setuptools.app.muledump.totals.menu.settings = function() {
    ...
    */
    ...
    setuptools.lightbox.build('totalsmenu-settings', ' \
        ...
        <div class="w100 flex-container scrollbar" style="height: 100px; overflow-y: scroll; flex-wrap: wrap;" id="itemSlotTypeSorting">\
    ');
    ...
    types.filter(function(type) {
        ...
        var classes = '';
        console.log(type, setuptools.data.options['totalsFilter-' + type]);
        ...
        setuptools.lightbox.build('totalsmenu-settings', ' \
            <div class="itemSlotTypeSorting cell flex-container noFlexAutoWidth ' + classes + '" data-type="' + type + '" style="overflow: hidden; justify-content: space-between;"> \
                <div>' + name + '</div> \
                <div style="width: 40px; height: 40px; position: relative; background-image: url(lib/renders.png); background-position: -' + item[3] + 'px -' + item[4] + 'px; background-repeat: no-repeat;"></div>\
            </div>\
        ');
    ...
    $('.itemSlotTypeSorting.cell')
        ...
        .on('dblclick.muledump.totals.itemGroupSorting', function(e) {
            ...
            setuptools.lightbox.build('totalsmenu-itemSorting', ' \
                ...
                <div class="w100 scrollbar flex-container" style="max-height: 440px; overflow-y: scroll; flex-wrap: wrap; justify-content: flex-start;" id="itemSubsortingBox">\
            ');
    ...
    $('div.setuptools.link.resetSlotOrder').on('click.muledump.resetSlotOrder', function() {
        setuptools.app.muledump.totals.config.setKey('slotOrder', setuptools.copy.totals.defaultConfig.slotOrder);
        setuptools.app.muledump.totals.menu.settings();
    });

    //  erase the hidden items list
    $('div.setuptools.link.clearAllHidden').on('click.muledump.clearAllHidden', function() {
        setuptools.app.muledump.totals.config.setKey('itemFilter', setuptools.copy.data.config.totalsKeyObjects.itemFilter);
...
setuptools.app.muledump.totals.menu.main = function(track) {
    ...
    if ( setuptools.data.options.totals === true ) {
        ...
        if ( Array.isArray(setuptools.app.muledump.totals.config.getKey('accountFilter')) && setuptools.app.muledump.totals.config.getKey('accountFilter').length > 0 ) options.push({
            ...
            callback: function() {
                setuptools.app.muledump.totals.config.setKey('accountFilter', setuptools.copy.data.config.totalsKeyObjects.accountFilter);
...
setuptools.app.muledump.itemsByVst = function(vst) {
    ...
    Object.filter(window.items, function(itemid, item) {
        if ( item[setuptools.config.vstIndex] === vst && item[3] !== 40 && item[4] !== 0 ) itemList.push(itemid);
    });
    return itemList;
...
setuptools.app.muledump.mulemenu = function(e, guid, context, track, extraopts) {
    ...
    if ( setuptools.data.options.totals === true ) options.push({
        ...
        callback: function(guid) {
            ...
            if ( typeof setuptools.app.muledump.totals.config.getKey('accountFilter') === 'undefined' ) setuptools.app.muledump.totals.config.setKey('accountFilter', setuptools.copy.data.config.totalsKeyObjects.accountFilter);
