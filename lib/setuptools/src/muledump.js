...
setuptools.app.muledump.mulemenu = function(e, guid, context, track, extraopts) {
    ...
    options.push({
        class: 'openWardrobe',
        name: 'Skin Wardrobe',
        callback: setuptools.app.muledump.ownedSkins,
        callbackArg: guid
    },{
        class: 'openWhitebags',
        name: 'Whitebag Tracker',
        callback: setuptools.app.muledump.whitebag.tracker,
        callbackArg: guid
    },{
        class: 'copyMenuOpen',
        name: "Copy...",
 ...
 */
setuptools.app.muledump.whitebag.init = function(guid, force) {
    ...
    if ( typeof config.accounts[guid] === 'object' && force === true ) config.accounts[guid].items = {};
...
setuptools.app.muledump.whitebag.save = function(guid, config) {
    ...
    setuptools.app.config.save('Whitebags/Totals saving changes');
...
setuptools.app.muledump.whitebag.tracker = function(guid) {
    ...
    setuptools.lightbox.build('whitebag-totals', ' \
        <div class="wbmenu flex-container">\
            <div class="muledump link save menuStyle positive menuTiny textCenter">Save</div>\
            <div class="muledump link reset menuStyle negative menuTiny textCenter">Reset</div>\
            <div class="muledump link cancel menuStyle disabled menuTiny textCenter">Cancel</div>\
            <div class="muledump link clear menuStyle negative menuTiny textCenter">Clear</div>\
            <div class="muledump link export menuStyle menuTiny textCenter">Export</div>\
            <div class="muledump link import menuStyle notice menuTiny textCenter">Import</div>\
        </div>\
        <div class="flex-container" style="background-color: #222;">\
            <div class="wbstage" style="background-color: #333;"> \
                ' + ( (mules[guid].opt('accountName') === true ) ? '<div class="header">' + setuptools.app.muledump.getName(guid) + '</div>' : '' ) + ' \
                <div class="items"><div style="background-color: #333;"></div></div> \
            </div> \
        </div>\
    ');
    ...
    setuptools.lightbox.drawhelp('whitebag-totals', 'docs/features/whitebags', 'White Bag Tracker Help');
    ...
    var wbitems = ((setuptools.app.muledump.whitebag.items()).filter(function(item) {
        ...
        //if ( items[a][1] === items[b][1] ) return (items[a][5]-items[b][5]);
    ...
    wbitems.filter(function(key) {
        ...
        var itemDom = window.item(key, 'wb noselect' + ( (selected === true) ? ' selected' : '' ), (config.items[key] || 0));
    ...
    wbitemsdom.on('click.wb.increment', function(e) {
        ...
        if ( e.shiftKey === true ) {
            ...
            if ( config.owned.indexOf(itemid) === -1 ) {
                ...
                if ( config.items[itemid] > 0 ) return;
        ...
        if ( value === 0 && e.ctrlKey === true ) return;
        value = ( e.ctrlKey === true ) ?
            (
                (value > 0) ?
                    value-1 :
                    value
    ...
    //  bind buttons (reset, cancel, save, screenshot, import)
    $('div.muledump.link.reset').on('click.wb.reset', function(e) {
    ...
    $('div.muledump.link.cancel').on('click.wb.cancel', function(e) {
    ...
    $('div.muledump.link.clear').on('click.wb.clear', function(e) {

        setuptools.app.muledump.whitebag.init(guid, true);
        setuptools.tmp.wbSaveOnClose = false;
        setuptools.app.muledump.whitebag.tracker(guid);

    });

    $('div.muledump.link.save').on('click.wb.save', function(e) {
    ...
    $('div.muledump.link.export').on('click.wb.export', function(e) {
        setuptools.app.muledump.exporter.ui.main(undefined, 'image', {
            items: 11,
            totals: config.items,
            sortorder: wbitems
        });
    ...
    $('div.muledump.link.import').on('click.wb.save', function(e) {
        ...
        setuptools.lightbox.build('whitebags-import', ' \
            ...
            \
            <div class="flex-container" style="justify-content: space-between; margin: 25px 0;">\
                <div class="muledump link import confirm menuStyle positive menuSmall textCenter">Yes, proceed</div>\
                <div class="muledump link import cancel menuStyle negative menuSmall textCenter">No, cancel</div>\
            </div>\
            \
            Ctrl+Click on \'Yes\' skip zeroing the counters\
        ');
        ...
        //  bind confirm button
        $('div.muledump.link.import.confirm').on('click.import.confirm', function(e) {

            //  ctrl+click overrides the zeroing of existing data
            if ( e.ctrlKey === false ) setuptools.app.muledump.whitebag.init(guid, true);
            var config = $.extend(true, {}, setuptools.data.muledump.whitebagTracker.accounts[guid]);
...
setuptools.app.muledump.exporter.ui.main = function(canvas, type, override) {
    ...
    function render(canvas) {
        ...
        if ( typeof canvas === 'string' ) {
            ...
            } else $('div.canvas.exporter').html('<img src="data:image/png;base64,' + canvas.split(',')[1] + '">');
    ...
    setuptools.lightbox.build('exporter-main', '\
        <div class="flex-container exporterMenu">\
            <div style="padding: 0 15px 0 0;"><strong>Totals Export</strong></div>\
            ...
            <div class="setuptools link canvas totals export remote imgur menuStyle menuSmall notice textCenter noclose mr0">Upload to  ' + ( (type === 'image' || typeof type === 'undefined') ? 'Imgur' : 'Paste' ) + '</div>\
    ...
    function procLocal(type) {
        ...
        if ( type === 'image' ) {
            ...
            var text = setuptools.app.muledump.exporter.text(type);
            //setuptools.app.ga('send', 'event', {
            //    eventCategory: 'detect',
            ...
            $('div.exporter.canvas').html('<textarea class="setuptools text exporter w100 scrollbar" style="height: 400px; margin: 15px 0 0 0;">' + text + '</textarea>');
    ...
    function procRemote(type) {
        ...
        if ( type === 'image' ) {
            ...
            if ( image.length === 0 || (image.length === 1 && image.attr('src').match(/^https?:\/\/.*/) !== null) ) {
                ...
                $('div.exporter.status').html('<br>Uploading to Imgur...<br>&nbsp;<br>');
        ...
        //  process paste uploads
            ...
            $('div.exporter.canvas').html('<textarea class="setuptools text exporter w100 scrollbar" style="height: 400px; margin: 0;">' + text + '</textarea>');
...
setuptools.app.muledump.charsort.menu = function(guid, charlist) {
    ...
    function charsortCellToggle(e, force) {
        ...
        if ( !e.shiftKey && force !== true ) return;
...
//  display owned skins
setuptools.app.muledump.ownedSkins = function(guid) {
    ...
    if ( typeof setuptools.tmp.skins !== 'object' ) {

        cache = false;
        setuptools.tmp.skins = {};

        //  generate the base object
        Object.filter(classes, function (classid, data) {
            if ( !Array.isArray(setuptools.tmp.skins[classid]) ) setuptools.tmp.skins[classid] = {
                total: 0,
                owned: 1,
                skins: []
            };
        ...
        Object.filter(skins, function(skinid, data) {
            if ( [65535].indexOf(data[4]) > -1 ) {
                count.total++;
                return;
            }
            setuptools.tmp.skins[data[4]].skins.push(Number(skinid));
        });

        //  sort each class
        Object.filter(setuptools.tmp.skins, function(classid, data) {
    ...
    Object.filter(setuptools.tmp.skins, function(classid, data) {
        ...
        var html = ' \
            <div style="justify-content: flex-start; flex-wrap: wrap;" class="flex-container ownedSkins ' + classData[0] + '"> \
                <div class="flex-container mb5" style="justify-content: space-between; padding: 5px; border: 2px solid #000000; background-color: #333;"> \
                    <div style="font-size: 14px;"><strong>' + classData[0] + '</strong></div>\
                    <div style="font-size: 14px; font-weight: bold;">[[' + classid + '_count]]</div>\
                </div>\
        ';
        ...
        data.skins.filter(function(skinid) {
            ...
            var $s = $('<img class="ownedSkins" style="padding: 5px; margin: 2px 3px 3px 2px; border: 1px solid #222222; overflow: hidden;" data-skinid="'+ skinid + '">');
            window.portrait($s, classid, skinid, undefined, undefined, false);
            if ( cache === false ) setuptools.tmp.skins[classid].total++;
            count.total++;
            if ( ownedSkins.indexOf(skinid) > -1 ) {
                if ( cache === false ) setuptools.tmp.skins[classid].owned++;
                count.owned++;
                $s.addClass('enabled');
            }
            skinDom.append($s);
        ...
        //  insert skin count
        html = html.replace('[[' + classid + '_count]]', setuptools.tmp.skins[classid].owned + ' of ' + setuptools.tmp.skins[classid].total + ' (' + ( ((setuptools.tmp.skins[classid].owned/setuptools.tmp.skins[classid].total)*100).toFixed(0) ) + '%)');
    ...
    //  bind tooltips
    $('img.ownedSkins').each(function() {

        var skinid = Number($(this).attr('data-skinid'));
        setuptools.app.muledump.tooltip($(this), 'ownedSkinsTooltip', '<div style="padding: 5px 10px; color: #fff;">' + skins[skinid][0] + '</div>');
