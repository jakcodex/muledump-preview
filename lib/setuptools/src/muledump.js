...
setuptools.app.muledump.totals.menu.populateByType = function(options, types) {
    ...
    for ( var a = 0; a < types.length; a++ ) {
        ...
        var displayName = window.itemsSlotTypeMap[type].displayName || type[0].toUpperCase()+type.substr(1);
        options.push({
...
setuptools.app.muledump.totals.menu.settings = function() {
    ...
    setuptools.lightbox.build('totalsmenu-settings', ' \
        ...
        ' + ( (setuptools.app.muledump.totals.config.getKey('accountFilter', false, []).length === 0) ? '<div class="w100" style="justify-content: flex-start">No accounts in filter</div>' : '' ) + ' \
        <div class="w100 scrollbar flex-container" style="max-height: 264px; overflow-y: scroll; flex-wrap: wrap; " id="accountFilterList">\
    ');

    setuptools.app.muledump.totals.config.getKey('accountFilter', false, []).sort();
    setuptools.app.muledump.totals.config.getKey('accountFilter', false, []).filter(function(guid) {
    ...
    setuptools.lightbox.build('totalsmenu-settings', ' \
        ...
        ' + ( ( setuptools.app.muledump.totals.config.getKey('itemFilter', false,[]).length === 0 ) ? '<div class="w100" style="justify-content: flex-start">No items hidden</div>' : '' ) + '\
    ...
    if ( setuptools.app.muledump.totals.config.getKey('itemFilter', false,[]).length > 0 ) {

        setuptools.app.muledump.totals.config.getKey('itemFilter', false,[]).filter(function(itemid) {
    ...
    if ( setuptools.app.muledump.totals.config.getKey('itemFilter', false,[]).length > 0 ) setuptools.lightbox.build('totalsmenu-settings', ' \
...
setuptools.app.muledump.totals.config.activate = function(name, reset) {
    ...
    //  reset keys
    if ( name === 'Default' ) {
        console.log('blah', setuptools.copy.totals.defaultConfig);
...
setuptools.app.muledump.itemsSlotTypeMapper = function(type) {
    ...
    //id = Number(id);
    //if ( typeof items[id] === 'undefined' ) return -1;
    //var item = items[id];
...
setuptools.app.muledump.toggleoption = function(option, value) {
    ...
    if ( option && !value ) {
        ...
        //  check if this option is an alias
        //  note: nevermind this for now
        //var keyNames = setuptools.app.muledump.optionIsAlias(option);

        //  switch the option
        for ( var i = 0; i < option.length; i++) setuptools.data.options[option[i]] = !setuptools.data.options[option[i]];
