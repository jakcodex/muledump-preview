...
setuptools.app.muledump.tooltip = function() {
    ...
    //  tooltip
    function tooltip(self, ItemData) {

        if ( typeof ItemData !== 'object' || ItemData[0] === 'Empty Slot' ) return;

        //  build tooltip data
        //  four columns: [ bagTypeImage ] [ item name/feed power ] [ tier/fame bonus ] [ menu ]
        var html = '';

        //  column one
        var bagPosition = '0px 0px';
        if ( ItemData[7] === 1 ) bagPosition = '-20px -0px';
        if ( ItemData[7] === 2 ) bagPosition = '-40px -0px';
        if ( ItemData[7] === 3 ) bagPosition = '-60px -0px';
        if ( ItemData[7] === 4 ) bagPosition = '-20px -20px';
        if ( ItemData[7] === 5 ) bagPosition = '-40px -20px';
        if ( ItemData[7] === 6 ) bagPosition = '-20px -40px';
        if ( ItemData[7] === 7 ) bagPosition = '-0px -20px';
        if ( ItemData[7] === 8 ) bagPosition = '-60px -20px';
        if ( ItemData[7] === 9 ) bagPosition = '-0px -40px';
        html += ' \
            <div class="cfleft"> \
        ...
        html += ' \
            <div class="fleft" style="margin-left: 5px;"> \
                ' + ItemData[0] + ( ( ItemData[8] === true ) ? ' <span class="tooltip generic text fright">(SB)</span>' : '' ) + ' \
    ...
    function ParseItem(self) {

        var ItemData;
        try {
            ItemData = JSON.parse($(self).attr('data-json'));
        } catch (e) {}
        return ItemData;
