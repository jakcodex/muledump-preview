...
setuptools.app.muledump.totalsmenuSettings = function() {
    ...
    setuptools.lightbox.build('totalsmenu-settings', ' \
        <div class="w100 accountFilterList" style="justify-content: flex-start; font-size: 16px;"><strong>Active Account Filter Members</strong></div>\
        <div class="w100 scrollbar flex-container" style="max-height: 264px; overflow-y: scroll;" id="accountFilterList">\
        ' + ( (setuptools.data.muledump.totalsAccountFilter.length === 0) ? 'No accounts in filter' : '' ) + '\
    ');

    setuptools.data.muledump.totalsAccountFilter.sort();
    setuptools.data.muledump.totalsAccountFilter.filter(function(guid) {
    ...
    setuptools.lightbox.build('totalsmenu-settings', ' \
        <div class="w100" style="justify-content: flex-start; font-size: 16px;"><strong>Permanently Hidden Items</strong></div>\
        <div class="w100 scrollbar flex-container" style="max-height: 264px; overflow-y: scroll; flex-wrap: wrap; justify-content: flex-start;" id="hiddenItems">\
    ');

    if ( setuptools.data.muledump.totalsItemFilter.length === 0 ) {

        setuptools.lightbox.build('totalsmenu-settings', 'No items hidden.');

    } else {

        setuptools.data.muledump.totalsItemFilter.filter(function(itemid) {
    ...
    if ( setuptools.data.muledump.totalsItemFilter.length > 0 ) setuptools.lightbox.build('totalsmenu-settings', ' \
        <div class="w100" style="justify-content: flex-start;"><div class="setuptools link clearAllHidden menuStyle negative">Clear All Hidden Items</div></div> \
    ');
    ...
    $('div.setuptools.link.clearAllHidden').on('click.muledump.clearAllHidden', function() {
        setuptools.data.muledump.totalsItemFilter = [];
    ...
    itemsSelected.off('click.muledump.totals.hiddenItems').on('click.muledump.totals.hiddenItems', function(e) {
        if ( e.shiftKey === true ) {
            ...
            setuptools.data.muledump.totalsItemFilter.splice(setuptools.data.muledump.totalsItemFilter.indexOf(itemId), 1);
            $(this).remove();
            $('.tooltip').remove();
    ...
    $('#accountFilterList div').off('click.muledump.totals.accountFilterList').on('click.muledump.totals.accountFilterList', function(e) {
        if ( e.shiftKey === true ) {
            ...
            var guid = $(this).text();
            setuptools.data.muledump.totalsAccountFilter.splice(setuptools.data.muledump.totalsAccountFilter.indexOf(guid), 1);
            $(this).remove();
            setuptools.tmp.globalTotalsCounter.import('clearExcluded');
            setuptools.app.config.save('Updating account list filter', true);
            option_updated('totals');
            if ( setuptools.data.muledump.totalsAccountFilter.length === 0 ) {
...
setuptools.app.muledump.totalsmenu = function(track) {
    ...
    if ( setuptools.data.options.totals === true ) {
        ...
        if ( Array.isArray(setuptools.data.muledump.totalsAccountFilter) && setuptools.data.muledump.totalsAccountFilter.length > 0 ) options.push({
            class: 'resetAccountFilter',
            name: 'Reset Account Filter',
            callback: function() {
                setuptools.data.muledump.totalsAccountFilter = [];
...
setuptools.app.muledump.totalsHide = function(itemid) {
    ...
    var index = setuptools.data.muledump.totalsItemFilter.indexOf(itemid);
    if ( index === -1 ) {
        setuptools.data.muledump.totalsItemFilter.push(itemid);
    } else setuptools.data.muledump.totalsItemFilter.splice(index, 1);
...
setuptools.app.muledump.mulemenu = function(e, guid, context, track, extraopts) {
    ...
    if ( setuptools.data.options.totals === true ) options.push({
        class: 'toggleTotals',
        name: ((
            Array.isArray(setuptools.data.muledump.totalsAccountFilter) === false ||
            (
                Array.isArray(setuptools.data.muledump.totalsAccountFilter) === true &&
                setuptools.data.muledump.totalsAccountFilter.indexOf(guid) === -1
            )) ? 'Add to' : 'Remove from' ) + ' Totals Filter',
        callback: function(guid) {

            if ( typeof setuptools.data.muledump.totalsAccountFilter === 'undefined' ) setuptools.data.muledump.totalsAccountFilter = [];
            if ( setuptools.data.muledump.totalsAccountFilter.indexOf(guid) === -1 ) {
                setuptools.data.muledump.totalsAccountFilter.push(guid);
            } else setuptools.data.muledump.totalsAccountFilter.splice(setuptools.data.muledump.totalsAccountFilter.indexOf(guid), 1);
