...
setuptools.app.muledump.notices.display = function() {
    ...
    var queue = setuptools.app.muledump.notices.queue;
    console.log('q', typeof queue, Array.isArray(queue), queue.length, queue);
    ...
    noticeDom.unbind('click').click(function() {
        ...
        var options = [{
            option: 'pos',
            vpx: noticeDom.outerHeight(true)+5
        }];
    ...
    //  display the button
    noticeDom.html('notice &nbsp;<span style="font-size: 14px; margin-top: 3px;">' + queue.length + '</span>').addClass('notifyPulse').show();
