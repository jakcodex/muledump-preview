...
setuptools.app.muledump.totals.menu.advanced = function(track, page) {
    ...
    if ( setuptools.data.muledump.totals.configSets.active !== 'Default' ) options.push({
        ...
        name: 'Save Settings',
        callback: function() {
            setuptools.app.muledump.totals.config.save();
    ...
    options.push({
        ...
        name: 'Reset Settings',
        callback: function() {
            setuptools.app.muledump.totals.config.activate();
        }
    });
...
setuptools.app.muledump.totals.menu.basic = function(track, page) {
    ...
    if ( page === 'consumables' ) setuptools.app.muledump.totals.menu.populateByType(
        options,
        ['candies', 'effusions', 'elixirs', 'helpfulconsumables', 'assistants', 'otherconsumables', 'potions', 'potionssb']
    );
    ...
    */
    ...
    setuptools.lightbox.build('totalsmenu-settings', ' \
        ...
        <div class="w100 flex-container" style="justify-content: space-around;">\
            ...
            <div class="flex-container noFlexAutoWidth" style="flex-basis: 326px; justify-content: flex-start; padding-left: 5px;">\
                <div class="setuptools link saveNewSet noclose menuStyle menuTiny positive textCenter flex-container" style="width: 124px; height: 26px;">Create</div>  \
            </div>\
        </div>\
        <div class="w100">&nbsp;</div>\
        \
        <div class="w100 itemSlotTypeSorting" style="justify-content: flex-start; font-size: 14px;"><strong>Item Sorting Order</strong></div>\
        ' + ( (setuptools.app.muledump.totals.config.getKey('slotOrder').length === 0) ? '<div class="w100" style="justify-content: flex-start">Uh oh :D</div>' : '' ) + ' \
        <div class="w100 flex-container scrollbar" style="height: 90px; overflow-y: scroll; flex-wrap: wrap;" id="itemSlotTypeSorting">\
    ');
    ...
    var types = setuptools.app.muledump.totals.config.getTypes().sort(function(a, b) {
        ...
        var vst = {};
        vst.a = window.itemsSlotTypeMap[a].virtualSlotType || window.itemsSlotTypeMap[a].slotType;
        vst.b = window.itemsSlotTypeMap[b].virtualSlotType || window.itemsSlotTypeMap[b].slotType;
    ...
    types.filter(function(type) {
        ...
        setuptools.lightbox.build('totalsmenu-settings', '<div class="itemSlotTypeSorting cell" data-type="' + type + '">' + name + '</div>');
    });
    ...
    setuptools.lightbox.drawhelp('totalsmenu-settings', 'docs/setuptools/help/totals-settings', 'Totals Help');
    ...
    if ( setuptools.state.loaded === false ) {
        ...
        $('.featherlight-content div.setuptools.menuStyle').addClass('disabled truly').removeClass('selected negative').attr('title', 'This feature requires SetupTools');
    }

    //  homegrown click and drag sorting of item groups!
    var MouseDirection = new Muledump_MouseDirection({
        target: {
            class: ['itemSlotTypeSorting', 'cell']
        }
    });
    $('.itemSlotTypeSorting.cell').on('mousedown.muledump.totals.itemSortingCells', function(e) {

        //  cell has been selected
        var self = $(this);
        $(window)
            //  move the cell with the mouse
            .off('mousemove.muledump.totals.itemSortingCells.movement')
            .on('mousemove.muledump.totals.itemSortingCells.movement', function(e) {

                //  valid elements to consider have data-type attributes
                var target = $(e.target).attr('data-type');
                if ( target === undefined ) return;
                var indexModifier = 0;
                if ( MouseDirection.rel.h === true ) {
                    self.insertAfter('.itemSlotTypeSorting.cell[data-type="' + target + '"]');
                    indexModifier++;
                } else {
                    self.insertBefore('.itemSlotTypeSorting.cell[data-type="' + target + '"]');
                }

                if (
                    typeof window.itemsSlotTypeMap[self.attr('data-type')] !== 'object' ||
                    typeof window.itemsSlotTypeMap[target] !== 'object'
                ) return;
                var vst = window.itemsSlotTypeMap[self.attr('data-type')].virtualSlotType || window.itemsSlotTypeMap[self.attr('data-type')].slotType;
                var targetVst = window.itemsSlotTypeMap[target].virtualSlotType || window.itemsSlotTypeMap[target].slotType;
                if ( vst === targetVst ) return;
                var slotOrder = setuptools.app.muledump.totals.config.getKey('slotOrder');
                slotOrder.splice(slotOrder.indexOf(vst), 1);
                slotOrder.splice((slotOrder.indexOf(targetVst)+indexModifier), 0, vst);

            })
            //  react to the mouse being released
            .off('mouseup.muledump.totals.itemSortingCells.movement')
            .on('mouseup.muledump.totals.itemSortingCells.movement', function(e) {
                $(window)
                    .off('mousemove.muledump.totals.itemSortingCells.movement')
                    .off('mouseup.muledump.totals.itemSortingCells.movement');
            });
    ...
    $('div.setuptools.link.clearAllHidden').on('click.muledump.clearAllHidden', function() {
        setuptools.app.muledump.totals.config.setKey('itemFilter', []);
...
setuptools.app.muledump.totals.menu.main = function(track) {
    ...
    if ( setuptools.data.options.totals === true ) {
        ...
        if ( Array.isArray(setuptools.app.muledump.totals.config.getKey('accountFilter')) && setuptools.app.muledump.totals.config.getKey('accountFilter').length > 0 ) options.push({
            ...
            callback: function() {
                setuptools.app.muledump.totals.config.setKey('accountFilter', []);
...
setuptools.app.muledump.totals.config.getKey = function(key, configSet, defaultValue) {
    ...
    if ( typeof setuptools.data.muledump.totals.configSets.settings[configSet][key] === 'undefined' ) {
        ...
        return defaultValue || setuptools.config.totalsKeyObjects[key];
    }
...
setuptools.app.muledump.totals.config.delete = function(configSet) {
    ...
    if ( typeof configSet !== 'string' ) return;
    if ( setuptools.app.muledump.totals.config.exists(configSet) === false ) return;
    delete setuptools.data.muledump.totals.configSets.settings[configSet];

    //  reset default configuration
    if ( configSet === 'Default' ) setuptools.data.muledump.totals.configSets.settings[configSet] = $.extend(true, {}, setuptools.copy.totals.defaultConfig);
...
setuptools.app.muledump.totals.config.save = function(name, immediate) {
    ...
    if ( typeof name === 'undefined' ) name = setuptools.data.muledump.totals.configSets.active;
    if ( typeof name !== 'string' ) return;
    var slotOrder = setuptools.app.muledump.totals.config.getKey('slotOrder');
    var itemFilter = setuptools.app.muledump.totals.config.getKey('itemFilter');
...
setuptools.app.muledump.totals.config.activate = function(name, reset) {
    ...
    if ( typeof name === 'undefined' ) name = setuptools.data.muledump.totals.configSets.active || 'Default';
    if ( typeof name !== 'string' ) return;
    if ( typeof setuptools.data.muledump.totals.configSets.settings[name] !== 'object' ) return;
    ...
    if (
        reset === true &&
        typeof setuptools.copy.totals.configSets.settings[name] === 'object'
    ) setuptools.data.muledump.totals.configSets.settings[name] = $.extend(true, {}, setuptools.copy.totals.configSets.settings[name]);

    //  reset keys
    if ( name === 'Default' ) {
        $.extend(true, setuptools.data.muledump.totals.configSets.settings[name], setuptools.copy.totals.defaultConfig);
        Object.filter(setuptools.copy.totals.defaultConfig, function(key, value) {
            setuptools.data.options[key] = value;
        });
    } else Object.filter(setuptools.data.muledump.totals.configSets.settings[name], function(key, value) {
        setuptools.data.options[key] = value;
    });
    setuptools.data.muledump.totals.configSets.active = name;
...
setuptools.app.muledump.itemsSlotTypeMapper = function(type) {
    ...
    var filteredItems = Object.filter(itemList, undefined, function(itemid, item) {
        ...
        var configKeys = Object.keys(Object.filter(mapConfig, function(type, data) {
            if ( data.slotType === slotType ) return true;
        }));
...
setuptools.app.muledump.totals.updateSecondaryFilter = function() {
    ...
    for ( var i = setuptools.config.totalsFilterKeysIndex; i < setuptools.config.totalsSaveKeys.length; i++ ) {
        ...
        var vstList = window.itemsSlotTypeMap[type].virtualSlotType || window.itemsSlotTypeMap[type].slotType;
...
setuptools.app.muledump.toggleoption = function(option, value, skip) {
    ...
    if ( skip !== true ) option_updated(option);
...
setuptools.app.muledump.mulemenu = function(e, guid, context, track, extraopts) {
    ...
    if ( setuptools.data.options.totals === true ) options.push({
        ...
        callback: function(guid) {
            ...
            if ( typeof setuptools.app.muledump.totals.config.getKey('accountFilter') === 'undefined' ) setuptools.app.muledump.totals.config.setKey('accountFilter', []);
