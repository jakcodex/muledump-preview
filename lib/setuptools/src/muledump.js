...
setuptools.app.muledump.exporter.state = {
    canvasWait: false
};
...
setuptools.app.muledump.exporter.ui.main = function(canvas, type) {
    ...
    function render(canvas) {
        ...
        var html = $('<div> \
            <div class="exporter status" style="text-align: center;">\
        ' + (
        ...
        if ( typeof canvas === 'object' ) {
            html.find('div.canvas.content').html(canvas);
        }
        ...
        if ( typeof canvas === 'string' ) {
            ...
            if ( canvas.match(/^https?:\/\/.*$/) ) {
                html.find('div.canvas.content').html('<a href="' + canvas + '" target="_blank"><img src="' + canvas + '"></a>');
            } else html.find('div.canvas.content').html('<img src="data:image/png;base64,' + canvas.split(',')[1] + '">');

        }
        $('div.exporter.canvas').html(html);
    ...
    function procRemote(type) {
        ...
        if ( type === 'image' ) {
            ...
            if ( image.length === 0 || (image.length === 1 && image.attr('src').match(/^https?:\/\/.*/) !== null) ) {
                ...
                $('div.exporter.canvas').html('&nbsp;<br>Uploading to Imgur...');
                ...
                $('div.exporter.status').prepend('<br>Uploading to Imgur...<br>&nbsp;');
            ...
            setuptools.app.muledump.exporter.paste(type, text, function(data, err, errDesc) {
                ...
                var statusDom = $('<div class="exporter status" style="text-align: center;"></div>');
                statusDom.insertBefore('div.exporter.canvas');

                if ( typeof data === 'undefined' ) {
                    $('div.exporter.status').prepend('<br><strong class="negative">Error</strong>: Failed to upload to Paste<br>' + errDesc);
                    return;
                }
                ...
                var pl = $('div.exporter.status').find('a.paste.link');
                if ( pl.length === 1 ) {
                ...
                $('div.exporter.status').prepend('<br>Your Paste Link: <a href="' + data.data.result.link + '" target="_blank" class="paste link">' + data.data.result.link + '</a>');

            });

            $('div.exporter.canvas').html('<textarea class="setuptools text exporter w100 scrollbar" style="height: 400px; margin: 15px 0 0 0;">' + text + '</textarea>');

            //setuptools.app.muledump.exporter.pastebin(mode.val(), text, function(data, err, errDesc) {

            //    $('div.exporter.status').html('<br>Your Pastebin URL: blah');
            //    console.log(data);
...
setuptools.app.muledump.charsort.menu = function(guid, charlist) {
        ...
        if ( typeof charlist === 'undefined' ) {
            ...
            setuptools.lightbox.build('muledump-charsort', ' \
                ...
                <div style="width: 500px; display: flex; justify-content: flex-start; flex-wrap: wrap;">\
                    <input name="charsortNewList" class="setuptools" placeholder="Enter New List Name ..." style="width: 45%; margin-bottom: 5px;">\
            ...
            lists = Object.keys(setuptools.data.muledump.chsortcustom.accounts[guid].data);
            for ( var i in lists) {
        ...
        //  character sorting gui
            ...
            for ( var cid in chars ) {
                ...
                if ( chars.hasOwnProperty(cid) ) {
                    ...
                    portraitHtml += '<div class="cell charsort ' + ( ( list.indexOf(chars[cid].id) > -1 ) ? 'enabled' : '') + '" data-charid="' + chars[cid].id + '" style="cursor: pointer; width: 200px; flex-grow: 1; margin: 5px; padding: 4px 2px 2px 2px;">' + portraits[chars[cid].id].html() + '</div>';
            ...
            setuptools.lightbox.build('muledump-charsort', ' \
                <div id="charField" class="charField flex-container noFlexAutoWidth scrollbar" style="height: 300px; overflow-y: scroll; flex-wrap: wrap;">' + portraitHtml + '</div> \
    ...
    $('div.charsort.cell').on('click.charsort.toggleEnable', function() {
    ...
    $('div.muledump.link.editList').on('click.edit.list', function() {
        ...
        setuptools.app.muledump.charsort.menu(guid, $('select[name="charsortList"]').val());
