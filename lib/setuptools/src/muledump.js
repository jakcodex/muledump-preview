...
setuptools.app.muledump.exporter.ui.main = function(canvas, type) {
    ...
    setuptools.lightbox.build('exporter-main', '\
        ...
        <div class="exporter status" style="text-align: center;"></div>\
    ...
    function procRemote(type) {
        ...
        //  process paste uploads
            ...
            setuptools.app.muledump.exporter.paste(type, text, function(data, err, errDesc) {
                ...
                if ( typeof data === 'undefined' || data.data.error === true ) {
...
setuptools.app.muledump.charsort.create = function(guid, charlist) {
    ...
    if ( typeof setuptools.data.muledump.chsortcustom.accounts[guid].data[charlist] === 'undefined' ) setuptools.data.muledump.chsortcustom.accounts[guid].data[charlist] = [];
    return true;
...
setuptools.app.muledump.charsort.menu = function(guid, charlist) {
    ...
    if (
        ...
        //  character sorting gui
            ...
            if ( setuptools.data.muledump.chsortcustom.sort > -1 ) {
            ...
            for ( var cid in chars ) {
                ...
                if ( chars.hasOwnProperty(cid) ) {
                    ...
                    portraitHtml += '<div class="cell charsort ' + ( ( list.indexOf(chars[cid].id) > -1 ) ? 'enabled' : '') + '" data-charid="' + chars[cid].id + '" style="cursor: pointer; width: 200px; flex-grow: 0; margin: 5px; padding: 4px 2px 2px 2px;">' + portraits[chars[cid].id].html() + '</div>';
            ...
            setuptools.lightbox.build('muledump-charsort', ' \
                <div id="charField" class="charField flex-container noFlexAutoWidth scrollbar" style="max-height: 300px; overflow-y: scroll; flex-wrap: wrap; justify-content: flex-start;">' + portraitHtml + '</div> \
                ...
                <div class="flex-container" style="justify-content: space-between; align-items: flex-start;">\
                    ...
                    <div>\
                        <select class="setuptools" name="chsortmode">\
                            <option value="-1" ' + ( (setuptools.data.muledump.chsortcustom.sort === -1) ? 'selected' : '' ) + '>Hide Disabled Characters</option> \
    ...
    $('select[name="chsortmode"]').on('change.chsortmode', function() {
        ...
        setuptools.app.config.save("Muledump/Charsort saving sort mode", true);
        setuptools.app.muledump.charsort.menu(guid, charlist);

    });

    //   toggle character enable/disable state
    $('div.charsort.cell').on('click.charsort.toggleEnable', function(e) {
