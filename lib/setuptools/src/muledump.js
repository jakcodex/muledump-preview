...
setuptools.app.muledump.ocl.manager = function(profile, advanced, hardreset) {
        ...
        <div class="ocl manager">\
            <div class="ocl configuration">\
                ...
                <div data-ocl="path">\
                    ...
                    <div>\
                        <input name="path" class="setuptools input ocl" value="' + data.params.path + '" placeholder="Browser URL or Flash Projector exe path here" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false">\
                    </div>\
                </div>\
                <div data-ocl="client" style="' + ( (data.params.mode === 'browser') ? 'display: none;' : '' ) + '">\
                ...
                <div data-ocl="paths" style="' + ( (advanced === false) ? 'display: none;' : '' ) + '">\
    ...
    setuptools.lightbox.display('ocl-manager', {variant: 'fl-SettingsLarge'});

    var help = {
        default: 'Use this profile when none other is selected for an account',
        mode: 'Launch game using the browser or Flash Projector',
        path: ['<span class="strong">Flash Projector Mode</span>: Path to Flash Projector exe (e.g. C:\\flashplayer_16_sa.exe)', '<span class="strong">Browser Mode</span>: URL to launch the game'],
        client: 'URL or local path to game SWF file',
        admin: 'Run AutoIt with administrative privileges (may cause Windows UAC prompt)',
    ...
    $('strong.help').on('click.help', function() {
        ...
        if ( dom.length === 0 ) {
            ...
            pieces.forEach(function(piece) {
                $('<div data-help="ocl ' + param + '" style="display:none;" class="w100 help">').html(piece).insertAfter(parent).slideDown();
            });
            $(this).html('&#9660;');
    ...
    $('div.setuptools.link.ocl.delete').on('click.ocl.delete', function() {
        ...
        setuptools.lightbox.status($('div.setuptools.link.ocl.delete'), "Ok!");
    ...
    $('div.setuptools.link.ocl.save').on('click.ocl.save', function() {
        ...
        $('div.ocl.configuration select').each(function() {
            ...
            var param = $(this).attr('name');
            if ( param === 'mode' ) {
                data[param] = ( $(this).val() === '1' ) ? 'flash' : 'browser';
                return;
            }
        ...
        setuptools.app.muledump.ocl.manager(profile, advanced);
        setuptools.lightbox.status($('div.ocl.configuration div.setuptools.link.save'), 'Ok!');
...
setuptools.app.muledump.ocl.install = function() {
    ...
    setuptools.lightbox.build('ocl-install', ' \
        <div id="drawhelpBox">\
            ...
            <br>Go to the AutoIt <a href="https://www.autoitscript.com/site/autoit/downloads/" target="_blank">Downloads</a> page and download the latest AutoIt.\
            \
            <br><br><h3>Download One Click Login AutoIt Script</h3>\
            ' + ( (setuptools.state.hosted === false) ? ' \
                <br>You can find a copy of the One Click Login AutoIt script in your Muledump installation folder at <strong>lib\\mulelogin.au3</strong>.\
                <br>\
            ' : '' ) + '\
            \
            <br>The latest version of One Click Login is available on Github: \
            <br><a href="' + setuptools.config.url + '/lib/mulelogin.au3" target="_blank" download="mulelogin.au3">' + setuptools.config.url + '/lib/mulelogin.au3</a> \
            \
            <br><br><h3>Install One Click Login AutoIt Script</h3>\
            <br>Run <strong>mulelogin.au3</strong> with AutoIt.\
            ...
            <br>You\'re all set! \
            <br><br>Go back to the previous page and enable One Click Login. Preconfigured profiles for Browser and Flash Projector are provided, and you can update them however you wish. \
            <br><br>The default profile is Browser. You can choose profiles per-account in the Account Menu\
        </div>\
    ');
    setuptools.lightbox.settitle('ocl-install', 'One Click Login Installation Guide');
    setuptools.lightbox.drawhelp('ocl-install', 'docs/setuptools/help/ocl-install', 'OCL Installation Help');
...
setuptools.app.muledump.ocl.activate = function(profile) {
    ...
    var profiles = setuptools.data.muledump.ocl.configSets.settings;
    if ( !profile ) profile = setuptools.data.muledump.ocl.configSets.active;
...
setuptools.app.muledump.ocl.router = function(guid) {
    ...
    if ( !guid ) return setuptools.app.muledump.ocl.get();
...
setuptools.app.muledump.ocl.mulelink = function(guid) {
    ...
    if (guid.match(setuptools.config.regex.guid) !== null) return;

    //  build ocl runtime parameters
    var account = setuptools.data.accounts.accounts[guid];
    var profile = setuptools.app.muledump.ocl.router(guid);
    var ocl_params = setuptools.app.muledump.ocl.params(profile);

    //  if this account is a testing account let's update the hostnames
    if ( setuptools.data.accounts.accounts[guid].testing === true ) ocl_params = ocl_params.replace(/www.realmofthemadgod.com/g, 'test.realmofthemadgod.com')

    //  return the link
    return 'muledump:' + sodium.to_hex(guid) + '-' + sodium.to_hex(window.accounts[guid]) + '-' + ocl_params;
