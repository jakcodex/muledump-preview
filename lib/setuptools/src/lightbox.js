...
setuptools.lightbox.menu.paginate.create = function(PageList, ActionItem, ActionContainer, ActionSelector, ActionCallback, ActionContext, Modifiers) {
    ...
    response.html.menu = ' \
        <div class="setuptools div pageControls full"> \
            ...
            ' + ( (typeof response.Modifiers.pageButtons === 'object' && typeof response.Modifiers.pageButtons.html === 'string' ) ? response.Modifiers.pageButtons.html : '' ) + ' \
        </div> \
    ';
...
setuptools.lightbox.menu.search.searchExecute = function(response, searchTerm, skip) {
    ...
    //  build our PageList
    var PageList;
    if ( typeof response.Modifiers === 'object' && typeof response.Modifiers.search === 'object' ) {

        //  we need to convert the complex object to a simple one
        PageList = [];
        for ( i = 0; i < response.PageList.length; i++ )
            if ( typeof response.PageList[i][response.Modifiers.search.key] === 'string' )
                PageList.push(response.PageList[i][response.Modifiers.search.key]);
