...
setuptools.lightbox.findPosition = function(dom, track, customPos) {
    ...
    console.log('pos', position.left, width, window.innerWidth);

    //  adjust menu position if this is going to render right of the window (untested)
    if ( position.left+width >= window.innerWidth ) css.left = css.left-width+40;

    //console.log(customPos, css);
    tooltip.css(css);
