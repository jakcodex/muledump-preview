...
setuptools.lightbox.menu.context.create = function(name, nested, track, options, self) {
        ...
        if (options.hasOwnProperty(i)) {
            ...
            if (!options[i].option) {
                ...
                //  allow an option for closing on mouseleave
                    ...
                    doClose.delay = ( typeof options[i].delay === 'number' ) ? options[i].delay : 100;
    ...
    function findPosition() {
        ...
        //  watch for content and react if missing
        if ( typeof watch === 'object' ) {

            watch.track = $('.' + watch.class + '[' + watch.attr + '="' + watch.value + '"]');
            if ( watch.track.length === 0 ) {
                window.techlog('Lightbox/ContextMenu - findPosition - executing watch callback');
        ...
        if ( typeof position === 'object' && position.top === 0 && position.left === 0 ) {
            ...
            if ( !reposition ) {
                window.techlog('Lightbox/ContextMenu - findPosition - closing on reposition init');
                setuptools.lightbox.menu.context.close(name);
                return;
            }
            ...
            if ( typeof position !== 'object' ) {
                window.techlog('Lightbox/ContextMenu - findPosition - closing on reposition failure');
                setuptools.lightbox.menu.context.close(name);
                return;
            }
    ...
    $('.setuptools.div.menu.' + name + ' a.menuLink').click(setuptools.lightbox.menu.context.close);
                        ...
                        if (options.hasOwnProperty(a)) {
                            ...
                            if (MenuOption.hasClass(options[a].class)) {
                                ...
                                if (option === 'LinkOption') {
                                    ...
                                    if (typeof options[a].callback !== 'function') {

                                        if ( typeof doClose.callback === 'function' ) {
                                            setTimeout(doClose.callback, doClose.delay);
                                        } else setuptools.lightbox.menu.context.close();
                                        return;
    ...
    if ( doHoverClose ) {
        ...
        $('.setuptools.div.menu.' + name)
            .on('mouseleave', function() { setuptools.tmp.muleMenuMouseLeaveTimer = setTimeout(setuptools.lightbox.menu.context.close, doHoverClose); })
...
setuptools.lightbox.create = function(data, config, title, page) {
    ...
    if ( typeof data === 'string' ) {
        ...
        if ( typeof config !== 'object' ) {
            ...
            if ( typeof config.root === 'undefined' ) config.root = '#fix';
...
setuptools.lightbox.display = function(page, config) {
    ...
    //  create the lightbox from the build data
        ...
        if ( ['drawhelp', 'main', 'accounts-deepcopy-download', 'settings-eraseAll-complete', 'settings-erase-completed', 'browser-assistant'].indexOf(page) === -1 ) {
            var widthOffset = 60;
