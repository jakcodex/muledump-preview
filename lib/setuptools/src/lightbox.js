            ...
            if (!options[i].option) {
                ...
                //  allow an option for closing on mouseleave
                    ...
                    if ( options[i].action === 'close' ) {
                        doHoverClose = 500;
                        doHoverCloseTimer = 'menuHoverClose';
                        if ( $.isNumeric(options[i].value) ) doHoverClose = options[i].value;
                        if ( typeof options[i].name === 'string' ) doHoverCloseTimer = options[i].name;
                    }
...
setuptools.lightbox.menu.context.keyup = function(name, e, selectorSuffix) {
    ...
    console.log('keyup', '.setuptools.div.menu.' + name + ' > div.link.menuSelected > a' + selectorSuffix);
    //  submit on enter
    if ( e.keyCode === 13 ) {
