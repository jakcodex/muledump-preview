        ...
        if (options.hasOwnProperty(i)) {
            ...
            if (!options[i].option) {
                ...
                //  create an actual link
                    ...
                    html += '<div class="link"><a href="' + options[i].href + '" class="setuptools menuLink ' + options[i].class + '" ' + ( (options[i].target) ? 'target="' + options[i].target + '"' : '' ) + ' ' + featherlightAttr + '><div class="w100 h100">' + options[i].name + '</div></a></div>\n';
...
setuptools.lightbox.create = function(data, config, title, page) {
    ...
    if ( typeof data === 'string' ) {
        ...
        if ( typeof config !== 'object' ) {
            ...
            if ( typeof config.beforeOpen === 'undefined' ) config.beforeOpen = setuptools.lightbox.menu.context.close;
