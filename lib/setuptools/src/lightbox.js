...
setuptools.lightbox.menu.paginate.create = function(PageList, ActionItem, ActionContainer, ActionSelector, ActionCallback, ActionContext, SearchModifier) {
...
setuptools.lightbox.menu.search.searchExecute = function(response, searchTerm, skip) {
    ...
    //  build our PageList
    var PageList;
    if ( typeof response.SearchModifier === 'object' && response.SearchModifier.type === 'isObject' && typeof response.SearchModifier.key === 'string' ) {

        //  we need to convert the complex object to a simple one
        PageList = [];
        for ( i = 0; i < response.PageList.length; i++ )
            if ( typeof response.PageList[i][response.SearchModifier.key] === 'string' )
                PageList.push(response.PageList[i][response.SearchModifier.key]);

    } else PageList = response.PageList;
...
setuptools.lightbox.menu.search.bind = function(response, skip, altContainer, altPosition, altAdjustments, altBinding, keepName) {
    ...
    searchName.unbind('keyup').keyup(function(e) {
        ...
        if ( typeof response.PageList[0] === 'object' ) {

            PageList = [];
            for ( var i in response.PageList )
                if ( response.PageList.hasOwnProperty(i) )
                    PageList.push(response.PageList[i].username);
...
setuptools.lightbox.display = function(page, config) {
    ...
    //  create the lightbox from the build data
        ...
        if ( ['drawhelp', 'main', 'accounts-deepcopy-download'].indexOf(page) === -1 ) {
            var widthOffset = 60;
