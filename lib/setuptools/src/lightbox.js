...
setuptools.lightbox.menu.context.create = function(track, options, self) {
    ...
    //  build the menu
    var doKeyup = true;
    var menu = $('<div class="setuptools div menu noselect">');
    $('body').append(menu);
    var html = '';
    var customPos = {
        h: 'left',
...
setuptools.lightbox.menu.paginate.create = function(PageList, ActionItem, ActionContainer, ActionSelector, ActionCallback, ActionContext) {
    ...
    response.html.menu = ' \
        <div class="setuptools div pageControls"> \
            ...
            <div class="editor control customPage"><input name="customPage" value="' + (response.currentPage+1) + '"><span class="noselect"> of ' + (response.lastPage+1) + ' page' + ( ((response.lastPage+1) > 1) ? 's' : '' ) + '</span></div> \
    ...
    response.bind = function(ActionContextOptions) {
        ...
        var searchExecute = function(searchTerm) {
            ...
            setuptools.lightbox.menu.paginate.findPage(searchIndex, ActionContainer);
...
setuptools.lightbox.menu.paginate.findPage = function(searchIndex, ActionContainer) {
    ...
    response.lastPage = Math.ceil(PageList.length/setuptools.data.config.accountsPerPage);
