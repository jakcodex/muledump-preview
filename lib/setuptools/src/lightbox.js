...
setuptools.lightbox.menu.context.create = function(title, track, options, self) {
        ...
        if (options.hasOwnProperty(i)) {
            ...
            if (!options[i].option) {
                ...
                if (options[i].option === 'header') {
                    ...
                    html += '<div class="setuptools"><strong>' + options[i].value + '</strong></div>\n';
    ...
    $('.setuptools.div.menu').click(function() {
        setuptools.lightbox.menu.context.close();
    });
...
setuptools.lightbox.menu.paginate.create = function(PageList, ActionItem, ActionContainer, ActionSelector, ActionCallback, ActionContext) {
    ...
    response.bind = function() {
        ...
        searchName.keyup(function(e) {
            ...
            var menuSelected = $('.setuptools.div.menu > div.selected > a.searchExecuteByName');

            //  if empty or if keypress was escape we exit
            ...
            if ( e.keyCode === 13 ) {
                ...
                if ( menuSelected.length === 0 ) {
                    ...
                    searchExecute(menuSelected.text());
                }
            ...
            if ( e.keyCode === 38 || e.keyCode === 40 ) {
                ...
                if ( setuptools.tmp.contextMenuOpen === true ) {
                    ...
                    if ( menuSelected.length === 0 ) {
                        ...
                        var menuOptions = $('.setuptools.div.menu > div > a.searchExecuteByName');
                        menuSelected = ( e.keyCode === 38 ) ? menuOptions.last() : menuOptions.first();
                        menuSelected.parent().addClass('selected');
                        menuSelected.parent().siblings().removeClass('selected');

                    } else {

                        var menuSelector = $('.setuptools.div.menu > div > a.searchExecuteByName');
                        var busy = false;
                        menuSelector.each(function(index, element) {

                            var menuSelected = $(element);
                            if ( busy === false && menuSelected.parent().hasClass('selected') ) {

                                busy = true;
                                if ( e.keyCode === 38 ) {

                                    console.log('go back on ' + menuSelected.text(), index, menuSelector.length-1);
                                    if ( index === 0 ) {
                                    ...
                                    console.log('go forward on ' + menuSelected.text(), index, menuSelector.length-1);
            ...
            for ( i = 0; i < hits.exact.length; i++ )
                options.push({
                    class: 'searchExecuteByName',
                    name: hits.exact[i],
                    callback: searchExecute,
                    callbackArg: hits.exact[i]
                });

            if ( hits.exact.length === 0 ) options.push({
                class: 'cancel',
                name: 'None',
                callback: function() {}
            });
            ...
            //  display the menu
            setuptools.lightbox.menu.context.create('Exact Matches', position, options);
