...
setuptools.storage.write = function(key, value, skipPrefix) {
    ...
    try {
        var result = localStorage.setItem(key, setuptools.storage.compression.deflate(data));
    } catch(e) {
        console.log('e', e);
        return false;
    }
...
setuptools.storage.report = function(options){
    ...
    var maximum = ( setuptools.browser === 'electron' ) ? 10485760 : 10240;
    var warn = {};
    var used = 0;
    var dcused = 0;
    var keys = {};
    ...
    for( var key in window.localStorage ){
        ...
        if(window.localStorage.hasOwnProperty(key)){
            ...
            if ( setuptools.browser === 'electron' && window.localStorage.reserved.indexOf(key) > -1 ) continue;
