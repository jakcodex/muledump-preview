...
setuptools.storage.report = function(options){
    ...
    for( var key in window.localStorage ){
        ...
        if(window.localStorage.hasOwnProperty(key)){
            ...
            if ( typeof decompressed === 'string' ) dcused += Number(((decompressed.length * 2)/1024).toFixed(2));
...
' + ( ( warn.storageSpace ) ? 'Warning: ' + warn.storageSpace + '\n---------------------------------------------\n' : '' ) + ' \
    ...
    return {used: used, remaining: remaining, percentFree: percentFree, ratio: Number(((1-(used/dcused))*100).toFixed(2)), keys: keys, summary: data, warning: ( typeof warn.storageSpace === 'string' ), warningText: warn.storageSpace};
...
setuptools.storage.compression.deflate = function(input, confirm, force) {
    ...
    if ( typeof input !== 'string' ) {
        setuptools.app.techlog('Storage/Compression - failed to start compression: Only strings may be compressed');
        return input;
    }
